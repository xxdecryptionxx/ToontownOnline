...
global master, _unittest_reportflags
...
class DocTest:
    ...
    def __cmp__(self, other):
        ...
        return cmp((self.name, self.filename, self.lineno, id(self)), (other.name, other.filename, other.lineno, id(other)))
...
class DocTestParser:
    ...
    def parse(self, string, name = '<string>'):
        ...
        if min_indent > 0:
            continue
            string = _[1]([ l[min_indent:] for l in string.split('\n') ])
    ...
    def _parse_example(self, m, name, lineno):
        ...
        continue
        source = _[1]([ sl[indent + 4:] for sl in source_lines ])
        want = m.group('want')
        ...
        continue
        want = _[1]([ wl[indent:] for wl in want_lines ])
...
class DocTestFinder:
    ...
    def _filter(self, obj, prefix, base):
        if self._namefilter is not None:
            pass
        return self._namefilter(prefix, base)
    ...
    def _find(self, tests, obj, name, module, source_lines, globs, seen):
        ...
        if inspect.ismodule(obj) and self._recurse:
            for (valname, val) in getattr(obj, '__test__', { }).items():
                ...
                if not inspect.isfunction(val) and inspect.isclass(val) and inspect.ismethod(val) and inspect.ismodule(val) or isinstance(val, basestring):
        ...
        if inspect.isclass(obj) and self._recurse:
            for (valname, val) in obj.__dict__.items():
                ...
                if (inspect.isfunction(val) and inspect.isclass(val) or isinstance(val, property)) and self._from_module(module, val):
                    valname = '%s.%s' % (name, valname)
                    self._find(tests, val, valname, module, source_lines, globs, seen)
                    continue
...
class DocTestRunner:
    ...
    def _DocTestRunner__record_outcome(self, test, f, t):
...
def DocTestSuite(module = None, globs = None, extraglobs = None, test_finder = None, **options):
    ...
    for test in tests:
        ...
        suite.addTest(DocTestCase(test, **None))
...
def DocFileSuite(*paths, **kw):
    ...
    for path in paths:
        suite.addTest(DocFileTest(path, **None))
    ...
    for piece in DocTestParser().parse(s):
        if isinstance(piece, Example):
            ...
            if want:
                ...
                continue
                _[1] += [ '## ' + l for l in want.split('\n')[:-1] ]
            
        continue
        _[1] += [ _comment_line(l) for l in piece.split('\n')[:-1] ]
