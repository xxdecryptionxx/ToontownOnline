...
class Random(_random.Random):
    ...
    def gammavariate(self, alpha, beta):
        ...
        if alpha > 1.0:
            ...
            while True:
                u1 = random()
                if 9.9999999999999995e-008 < u1:
                    pass
                u1 < 0.99999990000000005
                if not 1:
                    continue
            ...
            while True:
                ...
                if p <= 1.0:
                    ...
                    x = -_log((b - p) / alpha)
                u1 = random()
                if not (p <= 1.0 or u1 > _exp(-x)) and p > 1 and u1 > pow(x, alpha - 1.0):
                    break
                    continue
            return x * beta
...
class WichmannHill(Random):
    ...
    def _WichmannHill__whseed(self, x = 0, y = 0, z = 0):
...
def _test_generator(n, func, args):
    ...
    t1 = time.time()
    print round(t1 - t0, 3), 'sec,'avg = total / nstddev = _sqrt(sqsum / n - avg * avg), 'avg %g, stddev %g, min %g, max %g' % (avg, stddev, smallest, largest)
