...
class Fixture(NodePath, FSM):
    ...
    def enterUsing(self, args = []):
        ...
        if args and args[0]:
            self.accept('arrow_left', self.adjustFov, [
                -0.5,
                0])
            self.accept('arrow_left-repeat', self.adjustFov, [
                -2,
                0])
            self.accept('arrow_right', self.adjustFov, [
                0.5,
                0])
            self.accept('arrow_right-repeat', self.adjustFov, [
                2,
                0])
            self.accept('arrow_down', self.adjustFov, [
                0,
                -0.5])
            self.accept('arrow_down-repeat', self.adjustFov, [
                0,
                -2])
            self.accept('arrow_up', self.adjustFov, [
                0,
                0.5])
            self.accept('arrow_up-repeat', self.adjustFov, [
                0,
                2])
    ...
    def exitUsing(self):
        ...
        self.show()
        if self.dirty:
            messenger.send('refresh-fixture', [
                self.id,
                self.pack()])
            self.dirty = False
...
class DistributedCamera(DistributedObject):
    ...
    def setFixtures(self, fixtures):
        ...
        messenger.send('recordingInProgress', [
            recordingInProgress])
    ...
    def testFixture(self, index):
        ...
        if fixture:
            fixture.request('Using', [
                True])
            self.accept('escape', self.stopTesting, [
                index])
