...
class Transitions:
    ...
    def irisIn(self, t = 0.5, finishIval = None):
        ...
        if t == 0:
            ...
            self.transitionIval = Sequence(LerpScaleInterval(self.iris, t, scale = 0.17999999999999999, startScale = 0.01), Func(self.iris.detachNode), name = self.irisTaskName)
            if finishIval:
    ...
    def loadLetterbox(self):
        if not self.letterbox:
            ...
            self.letterboxTop = DirectFrame(parent = self.letterbox, guiId = 'letterboxTop', relief = DGG.FLAT, state = DGG.NORMAL, frameColor = (0, 0, 0, 1), borderWidth = (0, 0), frameSize = (-1, 1, 0, 0.20000000000000001), pos = (0, 0, 0.80000000000000004), image = barImage, image_scale = (2.25, 1, 0.5), image_pos = (0, 0, 0.10000000000000001), image_color = (0.29999999999999999, 0.29999999999999999, 0.29999999999999999, 1), sortOrder = 0)
            self.letterboxBottom = DirectFrame(parent = self.letterbox, guiId = 'letterboxBottom', relief = DGG.FLAT, state = DGG.NORMAL, frameColor = (0, 0, 0, 1), borderWidth = (0, 0), frameSize = (-1, 1, 0, 0.20000000000000001), pos = (0, 0, -1), image = barImage, image_scale = (2.25, 1, 0.5), image_pos = (0, 0, 0.10000000000000001), image_color = (0.29999999999999999, 0.29999999999999999, 0.29999999999999999, 1), sortOrder = 0)
