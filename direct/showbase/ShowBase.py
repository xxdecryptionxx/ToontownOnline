...
class ShowBase(DirectObject.DirectObject):
    ...
    def __init__(self, fStartDirect = True, windowType = None):
        ...
        self._ShowBase__directStarted = False
        self._ShowBase__deadInputs = 0
        ...
        self._ShowBase__configAspectRatio = ConfigVariableDouble('aspect-ratio', 0).getValue()
        self._ShowBase__oldAspectRatio = None
    ...
    def getAspectRatio(self, win = None):
        if self._ShowBase__configAspectRatio:
            return self._ShowBase__configAspectRatio
        ...
        elif win == None or not hasattr(win, 'getRequestedProperties'):
            props = WindowProperties.getDefault()
        else:
    ...
    def _ShowBase__resetPrevTransform(self, state):
        PandaNode.resetAllPrevTransform()
        return Task.cont

    
    def _ShowBase__dataLoop(self, state):
        self.dgTrav.traverse(self.dataRootNode)
        return Task.cont

    
    def _ShowBase__ivalLoop(self, state):
    ...
    def _ShowBase__shadowCollisionLoop(self, state):
        if self.shadowTrav:
    ...
    def _ShowBase__collisionLoop(self, state):
        if self.cTrav:
    ...
    def _ShowBase__audioLoop(self, state):
        if self.musicManager != None:
    ...
    def _ShowBase__igLoop(self, state):
    ...
    def _ShowBase__igLoopSync(self, state):
        onScreenDebug.render()
        if self.recorder:
            self.recorder.recordFrame()
    ...
    def restart(self, clusterSync = False, cluster = None):
        self.shutdown()
        self.taskMgr.add(self._ShowBase__resetPrevTransform, 'resetPrevTransform', priority = -51)
        self.taskMgr.add(self._ShowBase__dataLoop, 'dataLoop', priority = -50)
        self._ShowBase__deadInputs = 0
        self.taskMgr.add(self._ShowBase__ivalLoop, 'ivalLoop', priority = 20)
        self.taskMgr.add(self._ShowBase__collisionLoop, 'collisionLoop', priority = 30)
        self.cluster = cluster
        if not clusterSync or cluster == None:
            self.taskMgr.add(self._ShowBase__igLoop, 'igLoop', priority = 50)
        else:
            self.taskMgr.add(self._ShowBase__igLoopSync, 'igLoop', priority = 50)
        self.taskMgr.add(self._ShowBase__audioLoop, 'audioLoop', priority = 60)
    ...
    def silenceInput(self):
        if not self._ShowBase__deadInputs:
            self._ShowBase__deadInputs = taskMgr.remove('dataLoop')
        

    
    def reviveInput(self):
        if self._ShowBase__deadInputs:
    ...
    def toggleShowVertices(self):
        ...
        if self.config.GetBool('round-show-vertices', False):
            ...
            spot.renderSpot((1, 1, 1, 1), (0, 0, 0, 0), 0.80000000000000004, 1)
            tex = Texture('spot')
            tex.load(spot)
    ...
    def oobe(self):
        ...
        try:
            ...
            self.accept('oobe-down', self._ShowBase__oobeButton, extraArgs = [
                ''])
            self.accept('oobe-repeat', self._ShowBase__oobeButton, extraArgs = [
                '-repeat'])
            self.accept('oobe-up', self._ShowBase__oobeButton, extraArgs = [
                '-up'])
    ...
    def _ShowBase__oobeButton(self, suffix, button):
    ...
    def saveSphereMap(self, namePrefix = 'spheremap.png', defaultFilename = 0, source = None, camera = None, size = 256, cameraMask = PandaNode.getAllCameraMask(), numVertices = 1000):
        ...
        fm.setSquareInscribed(1, 1.1000000000000001)
        fm.setReflection(1)
    ...
    def windowEvent(self, win):
        if win == self.win:
            ...
            if not self._ShowBase__configAspectRatio:
                aspectRatio = self.getAspectRatio()
    ...
    def _ShowBase__doStartDirect(self):
        if self._ShowBase__directStarted:
            return None
        ...
        if not self.config.GetBool('want-directtools', 0):
            pass
        fDirect = self.config.GetString('cluster-mode', '') != ''
        self.startDirect(fWantDirect = fDirect, fWantTk = fTk, fWantWx = fWx)

    
    def run(self):
        if (self.appRunner is None and self.appRunner.dummy or self.appRunner.interactiveConsole) and not (self.appRunner.initialAppImport):
            self.taskMgr.run()
