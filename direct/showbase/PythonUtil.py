...
global __report_indent, Functor, _ProfileResultStr, _serialGen, dtoolSuperBase, safeReprNotify, baseLine, ftype, __report_indent, exceptionLoggedNotify, globalPdb, traceCalled, superLogFile
__all__ = [
    'enumerate',
    'unique',
    'indent',
...
import direct.extensions_native.extension_native_helpers as direct
...
class Functor:
    ...
    def _do__call__(self, *args, **kargs):
        ...
        _kargs.update(kargs)
        return self._function(*self._args + args, **self._args + args)

    
    def _exceptionLoggedCreationStack__call__(self, *args, **kargs):
        
        try:
            return self._do__call__(*args, **args)
        except Exception:
            e = None
...
class Stack:
    ...
    def push(self, item):
        self._Stack__list.append(item)

    
    def top(self):
        return self._Stack__list[-1]
    ...
    def isEmpty(self):
        return len(self._Stack__list) == 0
...
class Queue:
    ...
    def push(self, item):
        self._Queue__list.append(item)

    
    def top(self):
        return self._Queue__list[0]
    ...
    def back(self):
        return self._Queue__list[-1]

    
    def pop(self):
        return self._Queue__list.pop(0)
    ...
    def isEmpty(self):
        return len(self._Queue__list) == 0

    
    def __len__(self):
        return len(self._Queue__list)



def unique(L1, L2):
    continue
    L2 = _[1]([ (k, None) for k in L2 ])
    continue
    return _[1]
...
def profileFunc(callback, name, terse, log = True):
    ...
    __builtin__.globalProfileResult = [
        None]
    ...
    def profileDecorator(f):
        ...
        def _profiled(*args, **kArgs):
            ...
            if category is None or ConfigVariableBool('want-profile-%s' % category, 0).getValue():
                return profileFunc(Functor(f, *args, **args), name, terse)
            else:
                return f(*args, **args)
...
def _profileOpen(filename, *args, **kArgs):
    if filename in profileFilenames:
        ...
        file = movedOpenFuncs[-1](filename, *args, **args)
    return file
...
class SerialNumGen:
    ...
    def __init__(self, start = None):
        ...
        self._SerialNumGen__counter = start - 1

    
    def next(self):
        self._SerialNumGen__counter += 1
        return self._SerialNumGen__counter
...
class DelayedFunctor:
    ...
    def _callFunctor(self):
        cb = Functor(self._functor, *self._args, **self._args)
        del self._functor
        del self._name
        del self._delay
        del self._args
...
class ArgumentEater:
    ...
    def __call__(self, *args, **kwArgs):
        self._func(*args[self._numToEat:], **args[self._numToEat:])
...
def pstatcollect(scope, level = None):
    ...
    try:
        if not __dev__:
            pass
        if not config.GetBool('force-pstatcollect', 0) or not scope:
            return decorator
...
def report(types = [], prefix = '', xform = None, notifyFunc = None, dConfigParam = []):
    ...
    try:
        ...
        if not dConfigParam:
            doPrint = True
        elif not isinstance(dConfigParam, (list, tuple)):
            dConfigParams = (dConfigParam,)
        else:
            dConfigParams = dConfigParam
        continue
        dConfigParamList = _[1]
    ...
    def decorator(f):
        ...
        def wrap(*args, **kwargs):
            ...
            if 'args' in types:
                continue
                continue
                [] += _[1] + [ x + ' = ' + '%s, ' % repr(y) for (x, y) in kwargs.items() ]
...
def recordCreationStack(cls):
    ...
    def __recordCreationStack_init__(self, *args, **kArgs):
        ...
        return self.__moved_init__(*args, **args)
...
class GoldenRectangle:
    ...
    getLongerEdge = staticmethod(getLongerEdge)
    
    def getShorterEdge(longer):
        return longer / GoldenRatio

    getShorterEdge = staticmethod(getShorterEdge)
...
class HotkeyBreaker:
    ...
    def addBreakKey(self, breakKey):
        if __dev__:
            self.do.accept(breakKey, self.breakFunc, extraArgs = [
                breakKey])
...
def flywheel(*args, **kArgs):
    for flywheel in makeFlywheelGen(*args, **args):
        pass
...
class MiniLog:
    ...
    def enterFunction(self, funcName, *args, **kw):
        continue
        continue
        rArgs = _[1] + [ x + ' = ' + '%s, ' % repr(y) for (x, y) in kw.items() ]
        if not rArgs:
            rArgs = '()'
        else:
            rArgs = '(' + reduce(str.__add__, rArgs)[:-2] + ')'
...
class MiniLogSentry:
    ...
    def __init__(self, log, funcName, *args, **kw):
        self.log = log
        if self.log:
            self.log.enterFunction(funcName, *args, **args)
...
def recordFunctorCreationStacks():
    global Functor
    getConfigShowbase = getConfigShowbase
    import pandac.PandaModules
...
class AlphabetCounter:
    ...
    def next(self):
        continue
        result = _[1]([ c for c in self._curCounter ])
        index = -1
        while True:
