    ...
    def findCollisionEntry(self, skipFlags = SKIP_NONE, startIndex = 0):
        ...
        for i in range(startIndex, self.getNumEntries()):
            ...
            if skipFlags & SKIP_BACKFACE and self.isEntryBackfacing(entry):
                continue
            if skipFlags & SKIP_CAMERA and camera in nodePath.getAncestors():
                continue
            if skipFlags & SKIP_UNPICKABLE and nodePath.getName() in self.unpickable:
                continue
            if base.direct and skipFlags & SKIP_WIDGET and nodePath.getTag('WidgetName') != base.direct.widget.getName():
                continue
            if base.direct and skipFlags & SKIP_WIDGET and base.direct.fControl and nodePath.getName()[2:] == 'ring':
                continue
