...
COA_MARKER_SF = 0.0074999999999999997
Y_AXIS = Vec3(0, 1, 0)
...
class DirectCameraControl(DirectObject):
    ...
    def __init__(self):
        ...
        self.keyEvents = [
            ...
            [
                ...
                self.homeCam],
            [
                'DIRECT-toggleMarkerVis',
                self.toggleMarkerVis],
            [
                'DIRECT-moveToFit',
                self.moveToFit],
            [
                ...
                self.pickNextCOA],
            [
                'DIRECT-orbitUprightCam',
                self.orbitUprightCam],
            [
                'DIRECT-uprightCam',
                self.uprightCam],
            [
                'DIRECT-spwanMoveToView-1',
                self.spawnMoveToView,
                1],
            [
                'DIRECT-spwanMoveToView-2',
                self.spawnMoveToView,
                2],
            [
                'DIRECT-spwanMoveToView-3',
                self.spawnMoveToView,
                3],
            [
                'DIRECT-spwanMoveToView-4',
                self.spawnMoveToView,
                4],
            [
                'DIRECT-spwanMoveToView-5',
                self.spawnMoveToView,
                5],
            [
                'DIRECT-spwanMoveToView-6',
                self.spawnMoveToView,
                6],
            [
                'DIRECT-spwanMoveToView-7',
                self.spawnMoveToView,
                7],
            [
                'DIRECT-spwanMoveToView-8',
                self.spawnMoveToView,
                8],
            [
    ...
    def mouseFlyStart(self, modifiers):
        if self.useMayaCamControls and modifiers == 4:
            ...
            if abs(base.direct.dr.mouseX) < 0.90000000000000002 and abs(base.direct.dr.mouseY) < 0.90000000000000002:
                self.coaMarker.hide()
                self.startT = globalClock.getFrameTime()
                self.startF = globalClock.getFrameCount()
                self.spawnXZTranslateOrHPanYZoom()
            elif abs(base.direct.dr.mouseX) > 0.90000000000000002 and abs(base.direct.dr.mouseY) > 0.90000000000000002:
    ...
    def OrthoZoomTask(self, state):
        ...
        factor = (base.direct.dr.mouseDeltaX - 1.0 * base.direct.dr.mouseDeltaY) * 0.10000000000000001
    ...
    def mouseRotateTask(self, state):
        ...
        if state.constrainedDir == 'y' and abs(base.direct.dr.mouseX) > 0.90000000000000002:
            deltaX = 0
            deltaY = base.direct.dr.mouseDeltaY
        elif state.constrainedDir == 'x' and abs(base.direct.dr.mouseY) > 0.90000000000000002:
        ...
        if base.direct.fShift:
            ...
            self.camManipRef.setPos(self.coaMarkerPos)
            self.camManipRef.setHpr(base.direct.camera, ZERO_POINT)
        elif base.direct.camera.getPos().getZ() >= 0:
            dirX = -1
        else:
            dirX = 1
    ...
    def computeCOA(self, entry):
        ...
        if self.fLockCOA:
            ...
            coa.assign(hitPt)
            if hitPtDist < 1.1000000000000001 * dr.near or hitPtDist > dr.far:
                coa.assign(self.coaMarker.getPos(base.direct.camera))
        ...
        if coaDist < 1.1000000000000001 * dr.near:
            coa.set(0, 100, 0)
            coaDist = 100
    ...
    def orbitUprightCam(self):
        ...
        if orbitAngle < 0.10000000000000001:
            return None
    ...
    def spawnMoveToView(self, view):
        ...
        if view == 1:
            ...
            hprOffset.set(135.0, -35.264000000000003, 0.0)
    ...
    def fitOnWidget(self, nodePath = 'None Given'):
        ...
        camY = base.direct.dr.near * 2.0 * maxScale / 0.29999999999999999 * maxDim
