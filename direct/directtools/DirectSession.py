...
class DirectSession(DirectObject):
    ...
    def __init__(self):
        ...
        self.selectedNPReadout = OnscreenText.OnscreenText(pos = (-1.0, -0.90000000000000002), bg = Vec4(1, 1, 1, 1), scale = 0.050000000000000003, align = TextNode.ALeft, mayChange = 1, font = self.font)
        useDirectRenderStyle(self.selectedNPReadout)
        self.selectedNPReadout.reparentTo(hidden)
        self.activeParentReadout = OnscreenText.OnscreenText(pos = (-1.0, -0.97499999999999998), bg = Vec4(1, 1, 1, 1), scale = 0.050000000000000003, align = TextNode.ALeft, mayChange = 1, font = self.font)
        useDirectRenderStyle(self.activeParentReadout)
        self.activeParentReadout.reparentTo(hidden)
        self.directMessageReadout = OnscreenText.OnscreenText(pos = (-1.0, 0.90000000000000002), bg = Vec4(1, 1, 1, 1), scale = 0.050000000000000003, align = TextNode.ALeft, mayChange = 1, font = self.font)
        ...
        if base.config.GetBool('want-vrpn', 0):
            DirectDeviceManager = DirectDeviceManager
            import direct.directdevices
        ...
        self.actionEvents = [
            ...
            [
                ...
                self.deselectAll],
            [
                'DIRECT-preDeselectAll',
                self.deselectAllCB],
            [
                'highlightAll',
                self.selected.highlightAll],
            [
                'preRemoveNodePath',
                self.deselect],
            [
            ...
            [
                ...
                self.deselect],
            [
                'SGE_Set Reparent Target',
                self.setActiveParent],
            [
                'SGE_Reparent',
                self.reparent],
            [
                'SGE_WRT Reparent',
                lambda np, s = self: s.reparent(np, fWrt = 1)],
            [
                'SGE_Flash',
                self.flash],
            [
            ...
            [
                ...
                self.oobe],
            [
                'DIRECT-toggleWidgetVis',
                self.toggleWidgetVis],
            [
                'DIRECT-toggleWireframe',
                base.toggleWireframe],
            [
                'DIRECT-toggleVisAll',
                self.selected.toggleVisAll],
            [
                'DIRECT-toggleTexture',
                base.toggleTexture],
            [
                'DIRECT-upAncestry',
                self.upAncestry],
            ...
            [
                ...
                self.lights.toggle],
            [
                'DIRECT-toggleCOALock',
                self.cameraControl.toggleCOALock],
            [
                'DIRECT-setActiveParent',
                self.doSetActiveParent],
            [
                'DIRECT-doWrtReparent',
                self.doWrtReparent],
            [
                'DIRECT-doReparent',
                self.doReparent],
        ...
        if base.wantTk:
            ...
            DirectSessionPanel = DirectSessionPanel
            import direct.tkpanels
    ...
    def enableModifierEvents(self):
        for event in self.modifierEvents:
            self.accept(event, self.inputHandler, [
                event])
    ...
    def inputHandler(self, input):
        if not hasattr(self, 'oobeMode') or self.oobeMode == 0:
            if base.direct.manipulationControl.fMultiView:
                if not self.fMouse1 or 'mouse1' not in input:
                    if (self.fMouse2 or 'mouse2' not in input or self.fMouse3) and 'mouse3' not in input:
                        if input.endswith('-up') or input not in self.modifierEvents:
                            return None
                        
                    
                if not self.fMouse1 == 0 or 'mouse1-up' in input:
                    if (self.fMouse2 == 0 or 'mouse2-up' in input or self.fMouse3 == 0) and 'mouse3-up' in input:
                        return None
                    
                if (self.fMouse1 and self.fMouse2 or self.fMouse3) and input[4:7] != base.direct.camera.getName()[:3] and input.endswith('-up'):
                    return None
                ...
                if len(possibleWinCtrls) == 1:
                    ...
                    for cWinCtrl in possibleWinCtrls:
                        if input.endswith('-up') and input not in self.modifierEvents or input not in self.mouseEvents or input in self.mouseEvents:
                            if input[4:7] == cWinCtrl.camera.getName()[:3]:
                                winCtrl = cWinCtrl
                            
                        input[4:7] == cWinCtrl.camera.getName()[:3]
                        if input[4:7] != cWinCtrl.camera.getName()[:3]:
                            winCtrl = cWinCtrl
                            continue
        ...
        if input in self.hotKeyMap.keys():
            ...
            messenger.send('DIRECT-mouse1', sentArgs = [
                modifiers])
            ...
            messenger.send('DIRECT-mouse2', sentArgs = [
                modifiers])
            ...
            messenger.send('DIRECT-mouse3', sentArgs = [
                modifiers])
        elif input == 'shift':
            self.fShift = 1
        elif input == 'shift-up':
        ...
        if not taskMgr.hasTaskNamed('resizeObjectHandles'):
            ...
            if dnp:
                if self.manipulationControl.fMultiView:
                    ...
                    sf = 0.074999999999999997 * nodeCamDist * math.tan(deg2Rad(direct.drList[3].fovV))
                    self.manipulationControl.widgetList[3].setDirectScalingFactor(sf)
                else:
                    nodeCamDist = Vec3(dnp.getPos(direct.camera)).length()
                    sf = 0.074999999999999997 * nodeCamDist * math.tan(deg2Rad(direct.drList.getCurrentDr().fovV))
    ...
    def selectCB(self, nodePath, fMultiSelect = 0, fSelectTag = 1, fResetAncestry = 1, fLEPane = 0, fUndo = 1):
        ...
        if dnp:
            messenger.send('DIRECT_preSelectNodePath', [
                dnp])
            if fResetAncestry:
            ...
            editTypes = self.manipulationControl.getEditTypes([
                dnp])
            ...
            messenger.send('DIRECT_selectedNodePath', [
                dnp])
    ...
    def setActiveParent(self, nodePath = None):
        ...
        messenger.send('DIRECT_activeParent', [
            self.activeParent])
    ...
    def fitOnNodePath(self, nodePath = 'None Given'):
        ...
        taskMgr.doMethodLater(0.10000000000000001, fitTask, 'manipulateCamera')
    ...
    def getAndSetName(self, nodePath):
        ...
        if newName:
            ...
            messenger.send('DIRECT_nodePathSetName', [
                nodePath,
                newName])
...
class DisplayRegionContext(DirectObject):
    ...
    def __init__(self, cam):
        ...
        self.mouseX = 0.0
        self.mouseY = 0.0
        self.orthoFactor = 0.10000000000000001
    ...
    def setOrientation(self):
        hpr = self.cam.getHpr()
        if (hpr[2] < 135 or hpr[2] > 45 or hpr[2] > 225) and hpr[2] < 315:
            self.isSideways = 1
        elif (hpr[2] > -135 or hpr[2] < -45 or hpr[2] < -225) and hpr[2] > -315:
            self.isSideways = 1
        else:
            self.isSideways = 0
...
__builtins__['direct'] = DirectSession()
