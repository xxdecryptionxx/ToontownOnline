...
class Interval(DirectObject):
    ...
    def getPlayRate(self):
        return self._Interval__playRate
    ...
    def stepPlay(self):
        ...
        if self._Interval__playRate >= 0:
            ...
            t = (now - self._Interval__clockStart) * self._Interval__playRate + self._Interval__endT
            if t >= self._Interval__startT:
                if self.isStopped():
                    self.privInitialize(t)
                else:
                    self.privStep(t)
            elif self._Interval__startTAtStart:
                if self.isStopped():
                    if self.getOpenEnded() or self._Interval__loopCount != 0:
                        self.privReverseInstant()
                ...
                self.privReverseInitialize(self._Interval__startT)
            else:
                self.privStep(self._Interval__startT)
            if self._Interval__endT == self._Interval__startT:
                self._Interval__loopCount += 1
            else:
                timePerLoop = (self._Interval__endT - self._Interval__startT) / -(self._Interval__playRate)
