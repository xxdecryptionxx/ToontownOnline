    ...
    def initialiseoptions(self, myClass):
        if self.__class__ is myClass:
            ...
            for name in keywords.keys():
                ...
                name[:index] not in self._dynamicGroups
...
class DirectGuiWidget(DirectGuiBase, NodePath):
    ...
    def __init__(self, parent = None, **kw):
        ...
        if self['scale']:
            scale = self['scale']
            if isinstance(scale, VBase3) and type(scale) == types.IntType or type(scale) == types.FloatType:
                self.setScale(scale)
            else:
    ...
    def setFrameTexture(self):
        ...
        if textures == None and isinstance(textures, Texture) or isinstance(textures, types.StringTypes):
    ...
    def printConfig(self, indent = 0):
        ...
        for child in self.getChildren():
            messenger.send(DGG.PRINT + child.getName(), [
                indent + 2])
