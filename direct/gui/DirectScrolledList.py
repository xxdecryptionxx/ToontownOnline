...
class DirectScrolledList(DirectFrame):
    ...
    def __init__(self, parent = None, **kw):
        ...
        self.incButton.bind(DGG.B1PRESS, self._DirectScrolledList__incButtonDown)
        self.incButton.bind(DGG.B1RELEASE, self._DirectScrolledList__buttonUp)
        self.decButton = self.createcomponent('decButton', (), None, DirectButton, (self,))
        self.decButton.bind(DGG.B1PRESS, self._DirectScrolledList__decButtonDown)
        self.decButton.bind(DGG.B1RELEASE, self._DirectScrolledList__buttonUp)
    ...
    def scrollTo(self, index, centered = 0):
        ...
        if len(self['items']) <= numItemsVisible:
            ...
            ret = 0
        elif self.incButton['state'] == DGG.DISABLED or self.decButton['state'] == DGG.DISABLED:
            self._DirectScrolledList__buttonUp(0)
