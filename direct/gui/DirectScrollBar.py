...
class DirectScrollBar(DirectFrame):
    ...
    def __init__(self, parent = None, **kw):
        optiondefs = (('pgFunc', PGSliderBar, None), ('state', DGG.NORMAL, None), ('frameColor', (0.59999999999999998, 0.59999999999999998, 0.59999999999999998, 1), None), ('range', (0, 1), self.setRange), ('value', 0, self._DirectScrollBar__setValue), ('scrollSize', 0.01, self.setScrollSize), ('pageSize', 0.10000000000000001, self.setPageSize), ('orientation', DGG.HORIZONTAL, self.setOrientation), ('manageButtons', 1, self.setManageButtons), ('resizeThumb', 1, self.setResizeThumb), ('command', None, None), ('extraArgs', [], None))
        if kw.get('orientation') in (DGG.VERTICAL, DGG.VERTICAL_INVERTED):
            optiondefs += (('frameSize', (-0.040000000000000001, 0.040000000000000001, -0.5, 0.5), None),)
        else:
            optiondefs += (('frameSize', (-0.5, 0.5, -0.040000000000000001, 0.040000000000000001), None),)
        ...
        if self.decButton['frameSize'] == None and self.decButton.bounds == [
            0.0,
            0.0,
            0.0,
            0.0]:
            ...
            if self['orientation'] == DGG.HORIZONTAL:
                self.decButton['frameSize'] = (f[0] * 0.050000000000000003, f[1] * 0.050000000000000003, f[2], f[3])
            else:
                self.decButton['frameSize'] = (f[0], f[1], f[2] * 0.050000000000000003, f[3] * 0.050000000000000003)
        
        if self.incButton['frameSize'] == None and self.incButton.bounds == [
            0.0,
            0.0,
            0.0,
            0.0]:
            f = self['frameSize']
            if self['orientation'] == DGG.HORIZONTAL:
                self.incButton['frameSize'] = (f[0] * 0.050000000000000003, f[1] * 0.050000000000000003, f[2], f[3])
            else:
                self.incButton['frameSize'] = (f[0], f[1], f[2] * 0.050000000000000003, f[3] * 0.050000000000000003)
    ...
    def _DirectScrollBar__setValue(self):
