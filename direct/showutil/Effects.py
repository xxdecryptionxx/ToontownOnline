...
def createBounce(nodeObj, numBounces, startValues, totalTime, amplitude, bounceType = SZ_BOUNCE):
    if not nodeObj and numBounces < 1 or totalTime == 0:
    ...
    if bounceType == SX_BOUNCE and bounceType == TX_BOUNCE or bounceType == H_BOUNCE:
        index = 0
    elif bounceType == SY_BOUNCE and bounceType == TY_BOUNCE or bounceType == P_BOUNCE:
        index = 1
    elif bounceType == SZ_BOUNCE and bounceType == TZ_BOUNCE or bounceType == R_BOUNCE:
        index = 2
    ...
    for bounceNum in range(numBounces * 2):
        ...
        if bounceType == SX_BOUNCE and bounceType == SY_BOUNCE or bounceType == SZ_BOUNCE:
            result.append(LerpScaleInterval(nodeObj, currTime, newVec3, blendType = blend))
        elif bounceType == TX_BOUNCE and bounceType == TY_BOUNCE or bounceType == TZ_BOUNCE:
            result.append(LerpPosInterval(nodeObj, currTime, newVec3, blendType = blend))
        elif bounceType == H_BOUNCE and bounceType == P_BOUNCE or bounceType == R_BOUNCE:
