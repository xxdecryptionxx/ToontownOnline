...
class DistCogdoCraneObject(DistributedSmoothNode.DistributedSmoothNode, FSM.FSM):
    ...
    def activatePhysics(self):
        if not self.physicsActivated:
            ...
            self.accept(self.collideName + '-floor', self._DistCogdoCraneObject__hitFloor)
            self.accept(self.collideName + '-goon', self._DistCogdoCraneObject__hitGoon)
            self.acceptOnce(self.collideName + '-headTarget', self._DistCogdoCraneObject__hitBoss)
            self.accept(self.collideName + '-dropPlane', self._DistCogdoCraneObject__hitDropPlane)
    ...
    def enterSlidingFloor(self, avId):
        ...
        if self.avId == base.localAvatar.doId:
            ...
            if self.wantsWatchDrift:
                taskMgr.add(self._DistCogdoCraneObject__watchDrift, self.watchDriftName)
