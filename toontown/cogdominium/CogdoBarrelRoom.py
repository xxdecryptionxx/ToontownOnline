...
class CogdoBarrelRoom:
    ...
    def getIntroInterval(self):
        ...
        track.append(Sequence(Func(camera.reparentTo, render), Func(camera.setPosHpr, self.model, -20.0, -87.900000000000006, 12.0, -30, 0, 0), Func(base.transitions.irisIn, 0.5), Wait(1.0), LerpHprInterval(camera, duration = 2.0, startHpr = Vec3(-30, 0, 0), hpr = Vec3(0, 0, 0), blendType = 'easeInOut'), Wait(2.5), LerpHprInterval(camera, duration = 3.0, startHpr = Vec3(0, 0, 0), hpr = Vec3(-45, 0, 0), blendType = 'easeInOut'), Wait(2.5)))
    ...
    def showRewardUi(self, results, callback = None):
        ...
        taskMgr.doMethodLater(CogdoBarrelRoomConsts.RewardUiTime, self._CogdoBarrelRoom__rewardUiTimeout, self.rewardUiTaskName, extraArgs = [
            callback])
        return (track, trackName)
