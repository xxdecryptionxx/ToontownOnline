...
class CogdoFlyingLocalPlayer(CogdoFlyingPlayer):
    ...
    def initOrthoWalker(self):
        orthoDrive = OrthoDrive(9.7780000000000005, maxFrameMove = 0.5, wantSound = True)
    ...
    def initLocalPlayerIntervals(self):
        ...
        self.deathInterval = Sequence(Func(self.resetVelocities), Parallel(Parallel(Func(self._deathSfx.play), LerpHprInterval(self.toon, 1.0, Vec3(720, 0, 0)), LerpFunctionInterval(self.toon.setScale, fromData = 1.0, toData = 0.10000000000000001, duration = 1.0), self.toon.posInterval(0.5, Vec3(0, 0, -25), other = self.toon)), Sequence(Wait(0.5), Func(base.transitions.irisOut))), Func(self.toon.stash), Wait(1.0), Func(self.toonSpawnFunc), name = '%s.deathInterval' % self.__class__.__name__)
    ...
    def initCollisions(self):
        ...
        self.accept('Flyer.cHeadCollSphere-enter-%s' % ceilingColl, self._CogdoFlyingLocalPlayer__handleHeadCollisionIntoCeiling)
        self.accept('Flyer.cHeadCollSphere-exit-%s' % ceilingColl, self._CogdoFlyingLocalPlayer__handleHeadCollisionExitCeiling)
        self.accept('Flyer.cFloorEventSphere-exit-%s' % floorColl, self._CogdoFlyingLocalPlayer__handleEventCollisionExitFloor)
        self.accept('Flyer.cRayNode-enter-%s' % floorColl, self._CogdoFlyingLocalPlayer__handleRayCollisionEnterFloor)
        self.accept('Flyer.cRayNode-again-%s' % floorColl, self._CogdoFlyingLocalPlayer__handleRayCollisionAgainFloor)
    ...
    def _CogdoFlyingLocalPlayer__handleHeadCollisionIntoCeiling(self, collEntry):
    ...
    def _CogdoFlyingLocalPlayer__handleHeadCollisionExitCeiling(self, collEntry):
        self.isHeadInCeiling = False
        self.surfacePoint = None
    ...
    def landOnPlatform(self, collEntry):
        ...
        if platform is not None:
            if not platform.isStartOrEndPlatform():
                taskMgr.doMethodLater(0.5, self.delayedLandOnPlatform, 'delayedLandOnPlatform', extraArgs = [
                    platform])
            elif platform.isEndPlatform():
                taskMgr.doMethodLater(1.0, self.delayedLandOnWinPlatform, 'delayedLandOnWinPlatform', extraArgs = [
                    platform])
    ...
    def _CogdoFlyingLocalPlayer__handleEventCollisionExitFloor(self, collEntry):
        fromNodePath = collEntry.getFromNodePath()
        intoNodePath = collEntry.getIntoNodePath()
        intoName = intoNodePath.getName()
        fromName = fromNodePath.getName()
    ...
    def updateControlVelocity(self, dt):
        ...
        if (not leftPressed or not rightPressed) and self.controlVelocity[0] > maxVal or self.controlVelocity[0] < minVal:
        ...
        if (not upPressed or not downPressed) and self.controlVelocity[1] > maxVal or self.controlVelocity[1] < minVal:
    ...
    def setPropellerState(self, propState):
        ...
        if self.propState != propState:
            ...
            if self.propState == CogdoFlyingLocalPlayer.PropStates.Normal:
                ...
                self._loopPropellerSfx(playRate = 0.69999999999999996, volume = 0.80000000000000004)
    ...
    def startHitFlyingToonInterval(self):
        ...
        self.startEnemyHitIval(Sequence(Parallel(LerpFunc(spinPlayer, fromData = 0.0, toData = 1.0, duration = Globals.Gameplay.HitKnockbackTime, blendType = 'easeInOut', extraArgs = [
            direction]), LerpPosInterval(self.toon, duration = Globals.Gameplay.HitKnockbackTime, pos = hitByEnemyPos + collVec, blendType = 'easeOut')), Func(self.request, 'FreeFly'), name = 'hitByLegalEagleIval-%i' % self.toon.doId))
