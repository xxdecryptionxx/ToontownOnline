...
class CogdoFlyingMemo(CogdoFlyingGatherableBase, CogdoMemo):
    ...
    def update(self, dt):
        ...
        if self.floatTimer < self.floatDuration:
            ...
            self.floatDuration = random.uniform(1.8999999999999999, 2.1000000000000001)
    ...
    def pickUp(self, toon, elapsedSeconds = 0.0):
        ...
        if prop != None:
            ...
            if elapsedSeconds < respawnTime:
                taskMgr.doMethodLater(respawnTime - elapsedSeconds, self.addPropeller, 'propeller-respawn-%i' % self.serialNum, extraArgs = [
                    prop])
            else:
...
class CogdoFlyingLevelFog:
    ...
    def __init__(self, level, color = Globals.Level.FogColor):
        self._level = level
        self.color = color
        fogDistance = self._level.quadLengthUnits * max(1, self._level.quadVisibiltyAhead * 0.20000000000000001)
