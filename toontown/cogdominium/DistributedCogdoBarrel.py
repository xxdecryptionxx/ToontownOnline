...
class DistributedCogdoBarrel(DistributedObject.DistributedObject):
    ...
    def delete(self):
        ...
        ToontownIntervals.cleanup(self._DistributedCogdoBarrel__pulseIvalName())
        if self.model:
    ...
    def handleEnterSphere(self, collEntry = None):
        base.playSfx(self.bumpSound, volume = 0.34999999999999998, node = self.model, listener = camera)
        self.d_requestGrab()
    ...
    def setGrab(self, avId):
        if avId == base.localAvatar.doId:
            ToontownIntervals.start(ToontownIntervals.getPulseIval(self.model, self._DistributedCogdoBarrel__pulseIvalName(), 1.1499999999999999, duration = 0.20000000000000001))
