...
class CogdoMazeBossCodeFrame(DirectFrame):
    ...
    def __init__(self, id, code, modelToCopy):
        ...
        self._label = DirectLabel(parent = self._bg, relief = None, scale = Globals.BossCodeFrameLabelScale, text = code, pos = (0, 0, -0.029999999999999999), text_align = TextNode.ACenter, text_fg = Globals.BossCodeFrameLabelNormalColor, text_shadow = (0, 0, 0, 0), text_font = ToontownGlobals.getSuitFont())
    ...
    def showNumber(self):
        ...
        ToontownIntervals.cleanup('boss_code%i' % self._id)
        ToontownIntervals.start(Sequence(Parallel(ToontownIntervals.getPresentGuiIval(self._bossIcon, '', startPos = (0, 0, -0.14999999999999999))), Wait(1.0), ToontownIntervals.getPulseLargerIval(self._bg, ''), name = 'boss_code%i' % self._id))
