...
class CogdoFlyingGame(DirectObject):
    ...
    def pickUp(self, toonId, pickupNum, elapsedTime = 0.0):
        ...
        if gatherable is not None:
            if (not gatherable.isPowerUp() or not gatherable.wasPickedUp() or gatherable.isPowerUp()) and not gatherable.wasPickedUpByToon(player.toon):
    ...
    def _CogdoFlyingGame__startUpdateTask(self):
        self._CogdoFlyingGame__stopUpdateTask()
        taskMgr.add(self._CogdoFlyingGame__updateTask, CogdoFlyingGame.UpdateTaskName, 45)
