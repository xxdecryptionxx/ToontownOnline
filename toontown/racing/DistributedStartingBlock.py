...
class DistributedStartingBlock(DistributedObject.DistributedObject, FSM):
    ...
    def announceGenerate(self):
        ...
        self.accept(self.uniqueName('enterStartingBlockSphere'), self._DistributedStartingBlock__handleEnterSphere)
        if __debug__:
            if self.testLOD:
    ...
    def setOccupied(self, avId):
        ...
        if self.avId == 0:
            ...
            if av != None:
                ...
                self.av.stopSmooth()
                self.placedAvatar = 0
                self.acceptOnce(self.av.uniqueName('disable'), self._DistributedStartingBlock__avatarGone)
    ...
    def makeGui(self):
        ...
        self.cancelButton = DirectGui.DirectButton(relief = None, scale = 0.67000000000000004, pos = (1.1599999999999999, 0, -0.90000000000000002), text = ('', TTLocalizer.FishingExit, TTLocalizer.FishingExit), text_align = TextNode.ACenter, text_fg = Vec4(1, 1, 1, 1), text_shadow = Vec4(0, 0, 0, 1), text_pos = (0.0, -0.12), textMayChange = 0, text_scale = 0.10000000000000001, image = (fishGui.find('**/exit_buttonUp'), fishGui.find('**/exit_buttonDown'), fishGui.find('**/exit_buttonRollover')), text_font = ToontownGlobals.getInterfaceFont(), command = self.d_requestExit)
    ...
    def generateKartAppearTrack(self):
        ...
        kartTrack = Parallel(Sequence(ActorInterval(self.av, 'feedPet'), Func(self.av.loop, 'neutral')), Sequence(Func(self.kart.setActiveShadow, False), Func(self.kart.reparentTo, self.av.rightHand), Wait(2.1000000000000001), Func(self.kart.wrtReparentTo, render), Func(self.kart.setShear, 0, 0, 0), Parallel(LerpHprInterval(self.kart, hpr = self.kartNode.getHpr(render), duration = 1.2), ProjectileInterval(self.kart, endPos = self.kartNode.getPos(render), duration = 1.2, gravityMult = 0.45000000000000001)), Wait(0.20000000000000001), Func(self.kart.setActiveShadow, True), Sequence(LerpScaleInterval(self.kart, scale = Point3(1.1000000000000001, 1.1000000000000001, 0.10000000000000001), duration = 0.20000000000000001), LerpScaleInterval(self.kart, scale = Point3(0.90000000000000002, 0.90000000000000002, 0.10000000000000001), duration = 0.10000000000000001), LerpScaleInterval(self.kart, scale = Point3(1.0, 1.0, 0.10000000000000001), duration = 0.10000000000000001), LerpScaleInterval(self.kart, scale = Point3(1.0, 1.0, 1.1000000000000001), duration = 0.20000000000000001), LerpScaleInterval(self.kart, scale = Point3(1.0, 1.0, 0.90000000000000002), duration = 0.10000000000000001), LerpScaleInterval(self.kart, scale = Point3(1.0, 1.0, 1.0), duration = 0.10000000000000001), Func(self.kart.wrtReparentTo, self.kartNode))))
        return kartTrack
    ...
    def generateToonJumpTrack(self):
        ...
        jumpTrack = Sequence(Parallel(toonJumpTrack, Sequence(Wait(1), toonSitTrack)), Func(self.av.setPosHpr, 0, 0.45000000000000001, -0.25, 0, 0, 0), Func(self.av.reparentTo, self.kart.toonSeat))
        return jumpTrack
    ...
    def generateToonReverseJumpTrack(self):
        ...
        def getToonJumpTrack(av, destNode):
            ...
            toonJumpTrack = Parallel(ActorInterval(av, 'jump'), Sequence(Wait(0.10000000000000001), Parallel(LerpHprInterval(av, hpr = getJumpHpr, duration = 0.90000000000000002), ProjectileInterval(av, endPos = getJumpDest, duration = 0.90000000000000002))))
            return toonJumpTrack
    ...
    def generateKartDisappearTrack(self):
        ...
        def getHoleTrack(hole, holeParent):
            holeTrack = Sequence(Wait(0.20000000000000001), Func(hole.setBin, 'shadow', 0), Func(hole.setDepthTest, 0), Func(hole.setDepthWrite, 0), Func(hole.reparentTo, holeParent), Func(hole.setPos, holeParent, Point3(0, 0.0, -0.59999999999999998)), ActorInterval(hole, 'hole', startTime = 3.3999999999999999, endTime = 3.1000000000000001), Wait(0.40000000000000002), ActorInterval(hole, 'hole', startTime = 3.1000000000000001, endTime = 3.3999999999999999))
            return holeTrack
        ...
        def getKartShrinkTrack(kart):
            pos = kart.getPos()
            pos.addZ(-1.0)
            kartTrack = Sequence(LerpScaleInterval(kart, scale = Point3(1.0, 1.0, 0.90000000000000002), duration = 0.10000000000000001), LerpScaleInterval(kart, scale = Point3(1.0, 1.0, 1.1000000000000001), duration = 0.10000000000000001), LerpScaleInterval(kart, scale = Point3(1.0, 1.0, 0.10000000000000001), duration = 0.20000000000000001), LerpScaleInterval(kart, scale = Point3(0.90000000000000002, 0.90000000000000002, 0.10000000000000001), duration = 0.10000000000000001), LerpScaleInterval(kart, scale = Point3(1.1000000000000001, 1.1000000000000001, 0.10000000000000001), duration = 0.10000000000000001), LerpScaleInterval(kart, scale = Point3(0.10000000000000001, 0.10000000000000001, 0.10000000000000001), duration = 0.20000000000000001), Wait(0.20000000000000001), LerpPosInterval(kart, pos = pos, duration = 0.20000000000000001), Func(kart.hide))
            return kartTrack
    ...
    def doExitToRaceTrack(self):
        ...
        scaleLerpIval = LerpScaleInterval(self.kartNode, scale = oldBlockScale * 0.20000000000000001, duration = 2.0)
        ...
        if self.av == base.localAvatar:
            ...
            taskMgr.doMethodLater(1.6000000000000001, self.bulkLoad, 'loadIt', extraArgs = [])
...
class DistributedViewingBlock(DistributedStartingBlock):
    ...
    def announceGenerate(self):
        ...
        DistributedObject.DistributedObject.announceGenerate(self)
        self.nodePath.reparentTo(render)
        self.accept(self.uniqueName('enterStartingBlockSphere'), self._DistributedViewingBlock__handleEnterSphere)
        if __debug__:
            if self.testLOD:
