...
class DistributedRace(DistributedObject.DistributedObject):
    ...
    def setEnteredRacers(self, avAndKarts):
        ...
        for i in avAndKarts:
            self.cr.relatedObjectMgr.requestObjects(i, allCallback = self._DistributedRace__gotKartAvatarLink)
    ...
    def enterStart(self):
        ...
        taskMgr.doMethodLater(waitTime, self.fsm.request, 'goToRacing', extraArgs = [
            'racing'])
    ...
    def loadFog(self):
        ...
        if base.wantFog:
            ...
            self.fog.setColor(Vec4(0.59999999999999998, 0.69999999999999996, 0.80000000000000004, 1.0))
            if self.isUrbanTrack:
    ...
    def setupGeom(self):
        ...
        for i in range(4000):
            ...
            self.curve.getPoint((i / 4000.0) * (self.curve.getMaxT() - 9.9999999999999994e-012), self.curvePoints[-1])
            self.curveTs.append((i / 4000.0) * (self.curve.getMaxT() - 9.9999999999999994e-012))
    ...
    def precomputeSideStreets(self):
        ...
        for i in range(self.barricadeSegments):
            ...
            self.curve.getPoint((i / self.barricadeSegments) * (self.curve.getMaxT() - 9.9999999999999994e-012), testPoint)
    ...
    def genArrow(self, pos, hpr, id):
        ...
        cs = CollisionTube(Point3(0.59999999999999998, -6, 0), Point3(0.59999999999999998, 54, 0), 4.7999999999999998)
        cs.setTangible(0)
        triggerEvent = 'imIn-' + trigger
        cn = CollisionNode(trigger)
        cn.addSolid(cs)
