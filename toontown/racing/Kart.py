...
class Kart(NodePath, ShadowCaster.ShadowCaster):
    ...
    def generateEngineStartTrack(self):
        ...
        track = Parallel(SoundInterval(self.kartStartSfx), Func(self.kartLoopSfx.play), LerpFunctionInterval(self.kartLoopSfx.setVolume, fromData = 0, toData = 0.40000000000000002, duration = length))
        return Sequence(track, Func(printVol))

    
    def generateEngineStopTrack(self, duration = 0):
        track = Parallel(LerpFunctionInterval(self.kartLoopSfx.setVolume, fromData = 0.40000000000000002, toData = 0, duration = duration))
        return track
