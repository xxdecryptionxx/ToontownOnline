...
class RaceGUI:
    ...
    def initRaceMode(self):
        ...
        self.mapLines.setScale(0.00025000000000000001 * RaceGlobals.TrackDict[self.race.trackId][6])
        ...
        self.placeLabelNum = DirectLabel(relief = None, pos = TTLocalizer.RGUIplaceLabelNumPos, text = '1', text_scale = 0.34999999999999998, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_font = ToontownGlobals.getSignFont())
        self.placeLabelNum.reparentTo(self.raceModeRoot)
        self.directObjList.append(self.placeLabelNum)
        self.placeLabelStr = DirectLabel(relief = None, pos = TTLocalizer.RGUIplaceLabelStrPos, text = TTLocalizer.KartRace_FirstSuffix, text_scale = 0.10000000000000001, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_font = ToontownGlobals.getSignFont())
        self.placeLabelStr.reparentTo(self.raceModeRoot)
        self.directObjList.append(self.placeLabelStr)
        self.lapLabel = DirectLabel(relief = None, pos = (1.1000000000000001, 0, 0.45000000000000001), text = '1/' + str(self.race.lapCount), text_scale = 0.10000000000000001, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_font = ToontownGlobals.getSignFont())
        self.lapLabel.reparentTo(self.raceModeRoot)
        self.directObjList.append(self.lapLabel)
        self.photoFinishLabel = DirectLabel(relief = None, pos = (0, 0, -0.10000000000000001), text = TTLocalizer.KartRace_PhotoFinish, text_scale = TTLocalizer.RGUIphotoFinish, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_font = ToontownGlobals.getSignFont())
        self.photoFinishLabel.hide()
        self.directObjList.append(self.photoFinishLabel)
        self.wrongWayLabel = DirectLabel(relief = None, pos = (1.1000000000000001, 0, 0.84999999999999998), text = TTLocalizer.KartRace_WrongWay, text_scale = 0.10000000000000001, text_fg = (0.94999999999999996, 0, 0, 1), text_font = ToontownGlobals.getSignFont())
        ...
        for x in range(self.race.lapCount):
            minLabel = DirectLabel(relief = None, pos = (interpolateFacePos((2.0 * x + 1) / self.race.lapCount * 2)[0] - 0.059999999999999998, 0, 0.83999999999999997), text = "0'", text_scale = 0.059999999999999998, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_font = ToontownGlobals.getSignFont(), text_align = TextNode.ARight)
            minLabel.reparentTo(self.raceModeRoot)
            self.directObjList.append(minLabel)
            secLabel = DirectLabel(relief = None, pos = (interpolateFacePos((2.0 * x + 1) / self.race.lapCount * 2)[0] + 0.059999999999999998, 0, 0.83999999999999997), text = "00''", text_scale = 0.059999999999999998, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_font = ToontownGlobals.getSignFont(), text_align = TextNode.ARight)
            secLabel.reparentTo(self.raceModeRoot)
            self.directObjList.append(secLabel)
            fractionLabel = DirectLabel(relief = None, pos = (interpolateFacePos((2.0 * x + 1) / self.race.lapCount * 2)[0] + 0.14000000000000001, 0, 0.83999999999999997), text = '00', text_scale = 0.059999999999999998, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_font = ToontownGlobals.getSignFont(), text_align = TextNode.ARight)
        ...
        self.gagPanel = DirectFrame(parent = self.raceModeRoot, relief = None, image = loader.loadModel('phase_6/models/karting/gag_panel'), image_scale = 0.25, pos = (-1.1299999999999999, 0, -0.5))
    ...
    def update(self, time):
        ...
        if not (localRacer.finished) and self.faceEndPos[0] - localRacer.face.getX() < nearDiff:
            for racerId in self.racerDict.keys():
                ...
                if not (racer.enabled) and racerId == localAvatar.doId or racer.face.getX() >= self.faceEndPos[0]:
                    continue
