...
class DistributedGolfCourse(DistributedObject.DistributedObject, FSM, DelayDeletable):
    ...
    def announceGenerate(self):
        ...
        self._DistributedGolfCourse__delayDelete = DelayDelete.DelayDelete(self, 'GolfCourse.self')
        self.request('Join')
        self.normalExit = 1
            ...
            for avId in self.avIdList:
                if not self.localAvId == avId:
                    ...
                    if av:
                        ...
                        tPanels.setPos(-1.1699999999999999, 0, toonPanelsStart + whichToon * tpDiff)
        ...
        if self.toonPanels and self.localAvId != avId:
            ...
            for x in range(len(self.avIdList)):
                if avId == self.avIdList[x] and y < len(self.toonPanels):
                    ...
                    exitedToon = DirectLabel(parent = self.toonPanels[y], relief = None, pos = (0, 0, 0.40000000000000002), color = (1, 1, 1, 1), text_align = TextNode.ACenter, text = TTLocalizer.GolferExited % toonPanel.av.getName(), text_scale = 0.070000000000000007, text_wordwrap = 6)
