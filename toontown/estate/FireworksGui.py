...
class FireworksGui(DirectFrame):
    ...
    def __init__(self, doneEvent, shootEvent):
        DirectFrame.__init__(self, relief = None, geom = DGG.getDefaultDialogGeom(), geom_color = (0, 0.5, 1, 1), geom_scale = (0.42999999999999999, 1, 1.3999999999999999), pos = (1.1000000000000001, 0, 0))
    ...
    def load(self):
        ...
        for i in Fireworks.colors.keys():
            ...
            buttonBg = DirectFrame(self, geom = DGG.getDefaultDialogGeom(), geom_scale = paddedHeight, geom_color = self.bgColor, pos = (0.14999999999999999, 0, 0.5 - (paddedHeight + 0.025000000000000001) * i), relief = None)
            self.initialiseoptions(buttonBg)
            button = DirectButton(buttonBg, image = (DGG.getDefaultDialogGeom(), DGG.getDefaultDialogGeom(), DGG.getDefaultDialogGeom()), relief = None, command = self._FireworksGui__handleColor, extraArgs = [
                i])
