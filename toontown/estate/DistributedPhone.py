...
class DistributedPhone(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def replacePhoneInterval(self, toon):
        ...
        scaleTime = 1.0
        interval = Sequence(Parallel(ActorInterval(self.model, phoneBackAnim), ActorInterval(toon, 'phoneBack'), Sequence(Wait(1.0), Func(self._DistributedPhone__receiverToPhone), Func(base.playSfx, self.hangUpSfx))), self.scaleInterval(scaleTime, localAvatar.getGeomNode().getScale()[2], blendType = 'easeInOut'), Func(toon.loop, 'neutral'))
        if self.origToonHpr:
