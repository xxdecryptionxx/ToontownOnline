...
class TrunkGUI(StateData.StateData):
    ...
    def load(self):
        ...
        def addFrame(posZ, text):
            return DirectFrame(parent = self.parentFrame, image = shuffleFrame, image_scale = halfButtonInvertScale, relief = None, pos = (0, 0, posZ), hpr = (0, 0, 3), scale = 1.2, frameColor = (1, 1, 1, 1), text = text, text_scale = 0.057500000000000002, text_pos = (-0.001, -0.014999999999999999), text_fg = (1, 1, 1, 1))

        
        def addButton(parent, scale, hoverScale, posX, command, extraArg):
            return DirectButton(parent = parent, relief = None, image = (shuffleArrowUp, shuffleArrowDown, shuffleArrowRollover, shuffleArrowDisabled), image_scale = scale, image1_scale = hoverScale, image2_scale = hoverScale, pos = (posX, 0, 0), command = command, extraArgs = [
                extraArg])

        self.hatFrame = addFrame(0.10000000000000001, TTLocalizer.TrunkHatGUI)
        self.hatLButton = addButton(self.hatFrame, halfButtonScale, halfButtonHoverScale, -0.20000000000000001, self.swapHat, -1)
        self.hatRButton = addButton(self.hatFrame, halfButtonInvertScale, halfButtonInvertHoverScale, 0.20000000000000001, self.swapHat, 1)
        self.glassesFrame = addFrame(-0.14999999999999999, TTLocalizer.TrunkGlassesGUI)
        self.glassesLButton = addButton(self.glassesFrame, halfButtonScale, halfButtonHoverScale, -0.20000000000000001, self.swapGlasses, -1)
        self.glassesRButton = addButton(self.glassesFrame, halfButtonInvertScale, halfButtonInvertHoverScale, 0.20000000000000001, self.swapGlasses, 1)
        self.backpackFrame = addFrame(-0.40000000000000002, TTLocalizer.TrunkBackpackGUI)
        self.backpackLButton = addButton(self.backpackFrame, halfButtonScale, halfButtonHoverScale, -0.20000000000000001, self.swapBackpack, -1)
        self.backpackRButton = addButton(self.backpackFrame, halfButtonInvertScale, halfButtonInvertHoverScale, 0.20000000000000001, self.swapBackpack, 1)
        self.shoesFrame = addFrame(-0.65000000000000002, TTLocalizer.TrunkShoesGUI)
        self.shoesLButton = addButton(self.shoesFrame, halfButtonScale, halfButtonHoverScale, -0.20000000000000001, self.swapShoes, -1)
        self.shoesRButton = addButton(self.shoesFrame, halfButtonInvertScale, halfButtonInvertHoverScale, 0.20000000000000001, self.swapShoes, 1)
        ...
        self.cancelButton = DirectButton(relief = None, image = (self.gui.find('**/CrtAtoon_Btn2_UP'), self.gui.find('**/CrtAtoon_Btn2_DOWN'), self.gui.find('**/CrtAtoon_Btn2_RLLVR')), pos = (0.14999999999999999, 0, -0.84999999999999998), command = self._TrunkGUI__handleCancel, text = ('', TTLocalizer.MakeAToonCancel, TTLocalizer.MakeAToonCancel), text_font = ToontownGlobals.getInterfaceFont(), text_scale = 0.080000000000000002, text_pos = (0, -0.029999999999999999), text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1))
        self.cancelButton.hide()
        self.rotateL = DirectButton(relief = None, pos = (-0.14999999999999999, 0, 0.84999999999999998), image = (guiArrowRotateUp, guiArrowRotateDown, guiArrowRotateUp, guiArrowRotateDown), image_scale = (-0.69999999999999996, 0.69999999999999996, 0.69999999999999996), image1_scale = (-0.80000000000000004, 0.80000000000000004, 0.80000000000000004), image2_scale = (-0.80000000000000004, 0.80000000000000004, 0.80000000000000004))
        self.rotateL.hide()
        self.rotateL.bind(DGG.B1PRESS, self._TrunkGUI__rotateLDown)
        self.rotateL.bind(DGG.B1RELEASE, self._TrunkGUI__rotateLUp)
        self.rotateR = DirectButton(relief = None, pos = (0.14999999999999999, 0, 0.84999999999999998), image = (guiArrowRotateUp, guiArrowRotateDown, guiArrowRotateUp, guiArrowRotateDown), image_scale = (0.69999999999999996, 0.69999999999999996, 0.69999999999999996), image1_scale = (0.80000000000000004, 0.80000000000000004, 0.80000000000000004), image2_scale = (0.80000000000000004, 0.80000000000000004, 0.80000000000000004))
        self.rotateR.hide()
        ...
        if self.isOwner:
            ...
            self.trashPanel = DirectFrame(parent = aspect2d, image = DGG.getDefaultDialogGeom(), image_color = (1, 1, 0.75, 0.80000000000000004), image_scale = (0.35999999999999999, 0, 1.2), pos = (-0.85999999999999999, 0, 0.10000000000000001), relief = None)
            
            def addTrashButton(posZ, text, extraArg):
                return DirectButton(parent = self.trashPanel, image = trashImage, relief = None, pos = (-0.089999999999999997, 0, posZ), command = self._TrunkGUI__handleDelete, text = text, extraArgs = [
                    extraArg], scale = (0.5, 0.5, 0.5), text_font = ToontownGlobals.getInterfaceFont(), text_scale = 0.12, text_pos = (0.29999999999999999, 0), text_fg = (0.80000000000000004, 0.20000000000000001, 0.20000000000000001, 1), text_shadow = (0, 0, 0, 1), textMayChange = 0)

            self.hatTrashButton = addTrashButton(0.5, TTLocalizer.TrunkDeleteHat, ToonDNA.HAT)
            self.glassesTrashButton = addTrashButton(0.20000000000000001, TTLocalizer.TrunkDeleteGlasses, ToonDNA.GLASSES)
            self.backpackTrashButton = addTrashButton(-0.10000000000000001, TTLocalizer.TrunkDeleteBackpack, ToonDNA.BACKPACK)
            self.shoesTrashButton = addTrashButton(-0.40000000000000002, TTLocalizer.TrunkDeleteShoes, ToonDNA.SHOES)
            self.button = DirectButton(relief = None, image = (self.gui.find('**/CrtAtoon_Btn1_UP'), self.gui.find('**/CrtAtoon_Btn1_DOWN'), self.gui.find('**/CrtAtoon_Btn1_RLLVR')), pos = (-0.14999999999999999, 0, -0.84999999999999998), command = self._TrunkGUI__handleButton, text = ('', TTLocalizer.MakeAToonDone, TTLocalizer.MakeAToonDone), text_font = ToontownGlobals.getInterfaceFont(), text_scale = 0.080000000000000002, text_pos = (0, -0.029999999999999999), text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1))
    ...
    def swapHat(self, offset):
        ...
        if self.hatChoice < 0 and self.hatChoice >= len(self.hats) or len(self.hats[self.hatChoice]) != 3:
    ...
    def swapGlasses(self, offset):
        ...
        if self.glassesChoice < 0 and self.glassesChoice >= len(self.glasses) or len(self.glasses[self.glassesChoice]) != 3:
    ...
    def swapBackpack(self, offset):
        ...
        if self.backpackChoice < 0 and self.backpackChoice >= len(self.backpacks) or len(self.backpacks[self.backpackChoice]) != 3:
    ...
    def swapShoes(self, offset):
        ...
        if self.shoesChoice < 0 and self.shoesChoice >= len(self.shoes) or len(self.shoes[self.shoesChoice]) != 3:
    ...
    def _TrunkGUI__handleDelete(self, which):
        ...
        self.verify.show()
        self.accept('verifyDone', Functor(self._TrunkGUI__handleVerifyDelete, which))
        messenger.send('wakeup')
