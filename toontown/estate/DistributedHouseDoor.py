    ...
    def announceGenerate(self):
        ...
        if self.doorType == DoorTypes.EXT_STANDARD:
            ...
            if house and house.house_loaded:
                ...
                self.acceptOnce('houseLoaded-%d' % self.houseId, self._DistributedHouseDoor__gotRelatedHouse)
        elif self.doorType == DoorTypes.INT_STANDARD:
            door = render.find('**/leftDoor;+s')
            if door.isEmpty():
                self.acceptOnce('houseInteriorLoaded-%d' % self.zoneId, self._DistributedHouseDoor__gotRelatedHouse)
            else:
                self._DistributedHouseDoor__gotRelatedHouse()
