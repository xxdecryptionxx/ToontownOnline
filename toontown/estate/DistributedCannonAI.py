...
class DistributedCannonAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def requestEnter(self):
        ...
        if self.avId == 0:
            ...
            self.acceptOnce(self.air.getAvatarExitEvent(avId), self._DistributedCannonAI__handleUnexpectedExit, extraArgs = [
                avId])
            self.acceptOnce('bootAvFromEstate-' + str(avId), self._DistributedCannonAI__handleBootMessage, extraArgs = [
                avId])
            self._DistributedCannonAI__startTimeout(CannonGlobals.CANNON_TIMEOUT)
        else:
    ...
    def _DistributedCannonAI__startTimeout(self, timeLimit):
        self._DistributedCannonAI__stopTimeout()
        self.timeoutTask = taskMgr.doMethodLater(timeLimit, self._DistributedCannonAI__handleTimeout, self.taskName('timeout'))
