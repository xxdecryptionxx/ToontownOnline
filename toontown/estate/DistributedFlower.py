    ...
    def showWiltOrBloom(self):
        ...
        if useWilted:
            ...
            for leafIndex in range(leaves.getNumPaths()):
                ...
                leaf.setColorScale(1.0, 0.29999999999999999, 0.10000000000000001, 1.0)
    ...
    def handlePicking(self):
        ...
        if self.isWilted():
            self.confirmDialog = TTDialog.TTDialog(style = TTDialog.YesNo, text = TTLocalizer.ConfirmWiltedFlower % {
                'plant': fullName }, command = self.confirmCallback)
        elif not self.isFruiting():
            self.confirmDialog = TTDialog.TTDialog(style = TTDialog.YesNo, text = TTLocalizer.ConfirmUnbloomingFlower % {
                'plant': fullName }, command = self.confirmCallback)
            ...
            if giveSkillUp:
                ...
                self.confirmDialog = TTDialog.TTDialog(style = TTDialog.YesNo, text = TTLocalizer.ConfirmNoSkillupFlower % {
                    'plant': fullName }, command = self.confirmCallback)
