...
class DistributedLawnDecor(DistributedNode.DistributedNode, NodePath, ShadowCaster.ShadowCaster):
    ...
    def loadModel(self):
        ...
        if __dev__:
            ...
            self.model.setScale(0.40000000000000002, 0.40000000000000002, 0.10000000000000001)
    ...
    def stick2Ground(self, taskfooler = 0):
        ...
        if queue.getNumEntries() > 0:
            ...
            for index in range(queue.getNumEntries()):
                entry = queue.getEntry(index)
                if recurseParent(entry.getIntoNode(), 'terrain_DNARoot'):
                    self.setZ(entry.getSurfacePoint(render)[2] + self.stickUp + 0.10000000000000001)
                    self.stickParts()
                    return Task.done
                    continue
    ...
    def stopCamIval(self, avId):
        track = Sequence()
        if avId == localAvatar.doId:
            track = Sequence(Func(base.localAvatar.unsetCameraPosForPetInteraction), Wait(0.80000000000000004), Func(base.localAvatar.enableSmartCameraViews))
