...
class DistributedTarget(DistributedObject.DistributedObject):
    ...
    def handleHitTarget(self, avId = None, vel = None):
        ...
        if vel:
            ...
            self.targetBounceTrack = Sequence(LerpPosInterval(self.geom, duration = 0.10000000000000001, pos = newPos, blendType = 'easeOut'), LerpPosInterval(self.geom, duration = 0.25, pos = springPos, blendType = 'easeOut'), LerpPosInterval(self.geom, duration = 0.20000000000000001, pos = pos, blendType = 'easeOut'))
    ...
    def _DistributedTarget__showOnscreenMessage(self, titleText, scoreText):
        ...
        if not self.onscreenMessage:
            self.onscreenMessage = DirectFrame(relief = None, geom = DGG.getDefaultDialogGeom(), geom_color = GlobalDialogColor, geom_scale = (12, 1, 3), pos = (0, 0, 0.80000000000000004), scale = 0.10000000000000001)
            titles = DirectLabel(parent = self.onscreenMessage, relief = None, text = titleText, text_fg = VBase4(0, 0, 0, 1), text_align = TextNode.ALeft, text_scale = 0.69999999999999996, pos = (-5.75, 0, 0.5))
            scores = DirectLabel(parent = self.onscreenMessage, relief = None, text = scoreText, text_fg = VBase4(1, 0, 0, 1), text_align = TextNode.ARight, text_scale = 0.69999999999999996, pos = (5.75, 0, 0.5))
