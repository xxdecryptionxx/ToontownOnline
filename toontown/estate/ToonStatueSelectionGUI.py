...
class ToonStatueSelectionGUI(DirectFrame):
    ...
    def __init__(self, doneEvent, specialBoxActive = False):
        ...
        DirectFrame.__init__(self, relief = None, state = 'normal', geom = DGG.getDefaultDialogGeom(), geom_color = ToontownGlobals.GlobalDialogColor, geom_scale = (1.5, 1.0, 1.0), frameSize = (-1, 1, -1, 1), pos = (0, 0, 0), text = instructions, text_wordwrap = 18, text_scale = 0.080000000000000002, text_pos = instructionsPos)
        self.initialiseoptions(ToonStatueSelectionGUI)
        self.doneEvent = doneEvent
        buttons = loader.loadModel('phase_3/models/gui/dialog_box_buttons_gui')
        okImageList = (buttons.find('**/ChtBx_OKBtn_UP'), buttons.find('**/ChtBx_OKBtn_DN'), buttons.find('**/ChtBx_OKBtn_Rllvr'))
        cancelImageList = (buttons.find('**/CloseBtn_UP'), buttons.find('**/CloseBtn_DN'), buttons.find('**/CloseBtn_Rllvr'))
        self.cancelButton = DirectButton(parent = self, relief = None, image = cancelImageList, pos = (-0.29999999999999999, 0, -0.34999999999999998), text = TTLocalizer.PlantingGuiCancel, text_scale = 0.059999999999999998, text_pos = (0, -0.10000000000000001), command = self._ToonStatueSelectionGUI__cancel)
        self.okButton = DirectButton(parent = self, relief = None, image = okImageList, pos = (0.29999999999999999, 0, -0.34999999999999998), text = TTLocalizer.PlantingGuiOk, text_scale = 0.059999999999999998, text_pos = (0, -0.10000000000000001), command = self._ToonStatueSelectionGUI__accept)
        buttons.removeNode()
        self.ffList = []
        self.friends = { }
        self.doId2Dna = { }
    ...
    def makeFamilyButton(self, familyId, familyName, colorCode):
        ...
        return DirectButton(relief = None, text = familyName, text_scale = 0.040000000000000001, text_align = TextNode.ALeft, text_fg = fg, text1_bg = self.textDownColor, text2_bg = self.textRolloverColor, text3_fg = self.textDisabledColor, textMayChange = 0, command = self._ToonStatueSelectionGUI__chooseFriend, extraArgs = [
            familyId,
            familyName])
    ...
    def _ToonStatueSelectionGUI__chooseFriend(self, friendId, friendName):
        ...
        if self.checkFamily(friendId):
            if friendId == base.localAvatar.doId:
                ...
                base.cr.getAvatarDetails(familyAvatar, self._ToonStatueSelectionGUI__handleFamilyAvatar, 'DistributedToon')
        else:
    ...
    def _ToonStatueSelectionGUI__handleFamilyAvatar(self, gotData, avatar, dclass):
