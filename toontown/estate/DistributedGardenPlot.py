...
class DistributedGardenPlot(DistributedLawnDecor.DistributedLawnDecor):
    ...
    def _DistributedGardenPlot__handleFlowerPlantingDone(self, willPlant = 0, recipeStr = '', special = -1):
    ...
    def popupFlowerPlantingGui(self):
        ...
        self.acceptOnce(self.plantingGuiDoneEvent, self._DistributedGardenPlot__handleFlowerPlantingDone)
    ...
    def _DistributedGardenPlot__handleItemPlantingDone(self, willPlant = 0, recipeStr = '', selectedSpecial = -1):
        self.ignore(self.plantingGuiDoneEvent)
        self.ignore('stoppedAsleep')
        self.plantingGui.destroy()
        self.plantingGui = None
    ...
    def popupItemPlantingGui(self):
        base.localAvatar.hideGardeningGui()
        self.acceptOnce(self.plantingGuiDoneEvent, self._DistributedGardenPlot__handleItemPlantingDone)
        self.plantingGui = PlantingGUI.PlantingGUI(self.plantingGuiDoneEvent, True)
        self.plantingGui.showFirstSpecial()
        self.accept('stoppedAsleep', self._DistributedGardenPlot__handleItemPlantingDone)

    
    def popupToonStatueSelectionGui(self, species):
        base.localAvatar.hideGardeningGui()
        self.acceptOnce(self.toonStatueSelectionDoneEvent, self._DistributedGardenPlot__handleToonStatueSelectionDone, extraArgs = [
            species])
        self.toonStatueSelectionGui = ToonStatueSelectionGUI.ToonStatueSelectionGUI(self.toonStatueSelectionDoneEvent, True)
        self.accept('stoppedAsleep', self._DistributedGardenPlot__handleToonStatueSelectionDone)
    ...
    def _DistributedGardenPlot__handleToonStatueSelectionDone(self, species, willPlant = 0, recipeStr = '', dnaCode = -1):
    ...
    def popupTreePlantingGui(self):
        base.localAvatar.hideGardeningGui()
        self.acceptOnce(self.plantingGuiDoneEvent, self._DistributedGardenPlot__handleTreePlantingDone)
        self.plantingGui = PlantTreeGUI.PlantTreeGUI(self.plantingGuiDoneEvent)
        self.accept('stoppedAsleep', self._DistributedGardenPlot__handleTreePlantingDone)

    
    def _DistributedGardenPlot__handleTreePlantingDone(self, willPlant = False, gagTrack = None, gagLevel = None):
        self.ignore(self.plantingGuiDoneEvent)
        self.ignore('stoppedAsleep')
        self.plantingGui.destroy()
        self.plantingGui = None
    ...
    def generatePlaceItemTrack(self, toon, item):
        ...
        placeItemTrack.append(Sequence(ActorInterval(toon, 'start-dig'), Parallel(ActorInterval(toon, 'loop-dig', loop = 1, duration = 5.1299999999999999), Sequence(Wait(0.25), SoundInterval(sound, node = toon, duration = 0.55000000000000004), Wait(0.80000000000000004), SoundInterval(sound, node = toon, duration = 0.55000000000000004), Wait(1.3500000000000001), SoundInterval(sound, node = toon, duration = 0.55000000000000004))), ActorInterval(toon, 'start-dig', playRate = -1), Func(toon.loop, 'neutral'), Func(toon.detachShovel)))
        if self.model:
            pos = self.model.getPos()
            pos.setZ(pos[2] - 1)
        ...
        if item:
            placeItemTrack.append(Sequence(Func(item.reparentTo, toon.rightHand), Wait(0.55000000000000004), Func(item.wrtReparentTo, render), Parallel(LerpHprInterval(item, hpr = self.getHpr(render), duration = 1.2), ProjectileInterval(item, endPos = self.getPos(render), duration = 1.2, gravityMult = 0.45000000000000001)), Func(item.removeNode)))
