...
class DistributedPartyTugOfWarActivity(DistributedPartyTeamActivity):
    ...
    def handleToonJoined(self, toonId):
        ...
        if toonId == base.localAvatar.doId:
            ...
            self.toonIdsToAnimIntervals[toonId] = Sequence(Func(toon.startPosHprBroadcast, 0.10000000000000001), Func(toon.b_setAnimState, 'run'), LerpPosHprInterval(toon, duration, targetPos, VBase3(targetH, 0.0, 0.0), other = self.root), Func(toon.stopPosHprBroadcast), Func(toon.b_setAnimState, 'neutral'))
    ...
    def handleToonExited(self, toonId):
        ...
        if toonId == base.localAvatar.doId:
            ...
            if toonId not in self.fallenToons:
                ...
                hopOffAnim = Sequence(Func(toon.startPosHprBroadcast, 0.10000000000000001), toon.hprInterval(0.20000000000000001, VBase3(targetH, 0.0, 0.0), other = self.root), Func(toon.b_setAnimState, 'jump', 1.0), Wait(0.40000000000000002), PartyUtils.arcPosInterval(0.75, toon, targetPos, 5.0, self.root), Func(toon.stopPosHprBroadcast), Func(toon.sendCurrentPosition), Func(self.hopOffFinished, toonId))
    ...
    def handleToonShifted(self, toonId):
        if toonId == base.localAvatar.doId:
            ...
            toon = self.getAvatar(toonId)
            targetPos = self.dockPositions[self.localToonTeam][self.localToonPosIndex]
            self.toonIdsToAnimIntervals[toonId] = Sequence(Wait(0.59999999999999998), Func(toon.startPosHprBroadcast, 0.10000000000000001), Func(toon.b_setAnimState, 'run'), toon.posInterval(0.5, targetPos, other = self.root), Func(toon.stopPosHprBroadcast), Func(toon.b_setAnimState, 'neutral'))
            self.toonIdsToAnimIntervals[toonId].start()
    ...
    def setUpRopes(self):
        ...
        if leftToonId in self.toonIdsToRightHands and rightToonId in self.toonIdsToRightHands:
            self.tugRopes[ropeIndex].setup(3, ((self.toonIdsToRightHands[leftToonId], (0, 0, 0)), (self.root, (0.0, 0.0, 2.5)), (self.toonIdsToRightHands[rightToonId], (0, 0, 0))), [
                0,
                0,
                0,
                1,
            ...
            if numToons > 1:
                for i in range(numToons - 1, 0, -1):
                    ...
                    if not self.toonIdsToRightHands.has_key(toon1):
                        ...
                        continue
                    if not self.toonIdsToRightHands.has_key(toon2):
                    ...
                    self.tugRopes[ropeIndex].setup(3, ((self.toonIdsToRightHands[toon1], (0, 0, 0)), (self.toonIdsToRightHands[toon1], (0, 0, 0)), (self.toonIdsToRightHands[toon2], (0, 0, 0))), [
                        0,
                        0,
                        0,
                        1,
    ...
    def tightenRopes(self):
        ...
        self.tugRopes[0].setup(3, ((self.toonIdsToRightHands[self.toonIds[PartyGlobals.TeamActivityTeams.LeftTeam][0]], (0, 0, 0)), (self.toonIdsToRightHands[self.toonIds[PartyGlobals.TeamActivityTeams.LeftTeam][0]], (0, 0, 0)), (self.toonIdsToRightHands[self.toonIds[PartyGlobals.TeamActivityTeams.RightTeam][0]], (0, 0, 0))), [
            0,
            0,
            0,
            1,
    ...
    def checkIfFallen(self, toonId):
        if hasattr(self, 'fallenToons') and toonId not in self.fallenToons:
            ...
            if toon:
                ...
                if (team == PartyGlobals.TeamActivityTeams.LeftTeam or curPos[0] > -2.0 or team == PartyGlobals.TeamActivityTeams.RightTeam) and curPos[0] < 2.0:
