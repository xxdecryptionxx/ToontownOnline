...
class DistributedPartyCannon(DistributedObject, Cannon):
    ...
    def activate(self):
        self.accept(self.getEnterCollisionName(), self._DistributedPartyCannon__handleToonCollisionWithCannon)
        Cannon.show(self)
        self.active = True
    ...
    def _DistributedPartyCannon__handleToonCollisionWithCannon(self, collEntry):
    ...
    def enterCannon(self, avId):
        if avId == base.localAvatar.doId:
            ...
            if not self.activity.hasPlayedBefore():
                ...
                self.acceptOnce(DistributedPartyCannonActivity.RULES_DONE_EVENT, self._DistributedPartyCannon__enableCannonControl)
            else:
        ...
        if self.cr.doId2do.has_key(avId):
            ...
            if toon:
                self.acceptOnce(toon.uniqueName('disable'), self._DistributedPartyCannon__avatarGone, extraArgs = [
                    avId])
                toon.stopSmooth()
    ...
    def _DistributedPartyCannon__enableCannonControl(self):
        ...
        self.d_setTimeout()
        self.accept(CannonGui.FIRE_PRESSED, self._DistributedPartyCannon__handleFirePressed)
