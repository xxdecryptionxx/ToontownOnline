...
class PartyPlanner(DirectFrame, FSM):
    ...
    def load(self):
        self.frame = DirectFrame(parent = aspect2d, geom = self.gui.find('**/background'), relief = None, scale = 0.84999999999999998, pos = (0.050000000000000003, 0.0, 0.10000000000000001))
    ...
    def _createNavButtons(self):
        self.quitButton = DirectButton(parent = self.frame, relief = None, geom = (self.gui.find('**/cancelButton_up'), self.gui.find('**/cancelButton_down'), self.gui.find('**/cancelButton_rollover')), command = self._PartyPlanner__acceptExit)
        self.nextButton = DirectButton(parent = self.frame, relief = None, geom = (self.gui.find('**/bottomNext_button/nextButton_up'), self.gui.find('**/bottomNext_button/nextButton_down'), self.gui.find('**/bottomNext_button/nextButton_rollover')), command = self._PartyPlanner__nextItem, state = DirectGuiGlobals.DISABLED)
        self.prevButton = DirectButton(parent = self.frame, relief = None, geom = (self.gui.find('**/bottomPrevious_button/previousButton_up'), self.gui.find('**/bottomPrevious_button/previousButton_down'), self.gui.find('**/bottomPrevious_button/previousButton_rollover')), command = self._PartyPlanner__prevItem, state = DirectGuiGlobals.DISABLED)
    ...
    def _PartyPlanner__createNametag(self, parent):
        if self.nametagGroup == None:
            ...
            self.nametagNP.setPosHprScale(nametagPos[0], 0, nametagPos[2], 0, 0, 0, 0.10000000000000001, 1, 0.10000000000000001)
            ...
            self.chatNP.setPosHprScale(chatPos[0], 0, chatPos[2], 0, 0, 0, 0.080000000000000002, 1, 0.080000000000000002)
    ...
    def _createDatePage(self):
        ...
        self.makePartyNowButton = DirectButton(parent = page, relief = None, geom = (self.gui.find('**/send_up'), self.gui.find('**/send_down'), self.gui.find('**/send_rollover')), text = TTLocalizer.PartyPlannerPartyNow, text_pos = (pos[0], pos[2]), text_scale = 0.050000000000000003, command = self._PartyPlanner__doMakePartyNow)
        curServerDate = base.cr.toontownTimeManager.getCurServerDateTime()
        self.calendarGuiMonth = CalendarGuiMonth(page, curServerDate, scale = 0.94999999999999996, pos = (-0.050000000000000003, 0.0, -0.33000000000000002), dayClickCallback = self._dayClickCallback, onlyFutureDaysClickable = True)
        return page
    ...
    def _createTimePage(self):
        ...
        self.timePageToontownLabel = DirectLabel(parent = page, relief = None, text = TTLocalizer.PartyPlannerTimeToontown, pos = self.gui.find('**/step_03_toontown_locator').getPos(), scale = 0.14999999999999999, text_fg = (1.0, 0.0, 0.0, 1.0), text_font = ToontownGlobals.getSignFont())
        self.timePageTimeLabel = DirectLabel(parent = page, relief = None, text = TTLocalizer.PartyPlannerTimeTime, pos = self.gui.find('**/step_03_time_locator').getPos(), scale = 0.14999999999999999, text_fg = (1.0, 0.0, 0.0, 1.0), text_font = ToontownGlobals.getSignFont())
        self.timePageRecapLabel = DirectLabel(parent = page, relief = None, text = TTLocalizer.PartyPlannerTimeRecap, pos = self.gui.find('**/step_03_partyDateAndTime_locator').getPos(), scale = 0.089999999999999997)
        self.timePageRecapToontownTimeLabel1 = DirectLabel(parent = page, relief = None, text = TTLocalizer.PartyPlannerTimeToontownTime, pos = self.gui.find('**/step_03_toontownTime_locator').getPos(), scale = 0.059999999999999998)
        self.timePageRecapToontownTimeLabel2 = DirectLabel(parent = page, relief = None, text = '%s' % PartyUtils.formatDateTime(self.partyTime), pos = self.gui.find('**/step_03_toontownDateAndTime_loactor').getPos(), textMayChange = True, scale = 0.059999999999999998)
        self.timePageRecapLocalTimeLabel = DirectLabel(parent = page, relief = None, text = '%s%s' % (TTLocalizer.PartyPlannerTimeLocalTime, PartyUtils.formatDateTime(self.partyTime, inLocalTime = True)), pos = self.gui.find('**/step_03_localDateAndTime_loactor').getPos(), textMayChange = True, scale = 0.059999999999999998, text_fg = (1.0, 0.0, 0.0, 1.0))
        ...
        self.timePagecolonLabel = DirectLabel(parent = page, relief = None, text = ':', pos = self.gui.find('**/step_03_colon_locator').getPos(), scale = 0.14999999999999999)
        return page
    ...
    def getTimeWidgets(self, page, type):
        ...
        upButton = DirectButton(parent = page, relief = None, geom = (self.gui.find('**/%sButtonUp_up' % type), self.gui.find('**/%sButtonUp_down' % type), self.gui.find('**/%sButtonUp_rollover' % type)), command = changeValue, extraArgs = [
            self,
            self.timeTypeToChangeAmount[type][0]])
        downButton = DirectButton(parent = page, relief = None, geom = (self.gui.find('**/%sButtonDown_up' % type), self.gui.find('**/%sButtonDown_down' % type), self.gui.find('**/%sButtonDown_rollover' % type)), command = changeValue, extraArgs = [
            self,
    ...
    def _createPartyEditorPage(self):
        ...
        self.LayoutTitleLabel = DirectLabel(parent = page, relief = None, text = TTLocalizer.PartyPlannerEditorTitle, pos = self.gui.find('**/title_locator').getPos() + Point3(0.0, 0.0, 0.074999999999999997), scale = self.titleScale)
        self.costLabel = DirectLabel(parent = page, pos = (-0.73999999999999999, 0.0, 0.17000000000000001), relief = None, text = TTLocalizer.PartyPlannerTotalCost % 0, text_align = TextNode.ACenter, scale = TTLocalizer.PPcostLabel, textMayChange = True)
        self.partyGridBackground = DirectFrame(parent = page, relief = None, geom = self.gui.find('**/partyGrid_flat'))
        self.partyGroundsLabel = DirectLabel(parent = page, relief = None, text = TTLocalizer.PartyPlannerPartyGrounds, text_font = ToontownGlobals.getSignFont(), text_fg = VBase4(1.0, 0.0, 0.0, 1.0), text_scale = TTLocalizer.PPpartyGroundsLabel, pos = self.gui.find('**/step_05_partyGrounds_text_locator').getPos(), scale = 0.10000000000000001)
        self.activityBackground = DirectFrame(parent = page, relief = None, geom = self.gui.find('**/activitiesDecorations_flat1'), pos = (0.0, 0.0, 0.040000000000000001))
        ...
        self.elementTitleLabel = DirectLabel(parent = page, relief = None, text = ' ', pos = self.gui.find('**/step_05_activitiesName_text_locator').getPos() + Point3(0.0, 0.0, 0.040000000000000001), text_scale = TTLocalizer.PPelementTitleLabel, textMayChange = True)
        ...
        self.elementPrice.setPos(self.gui.find('**/step_05_activityPrice_text_locator').getPos() + Point3(-0.02, 0.0, 0.040000000000000001))
        ...
        self.elementDescription.setPos(self.gui.find('**/step_05_activityDescription_text_locator').getPos() + Point3(0.0, 0.0, 0.040000000000000001))
        self.totalMoney = base.localAvatar.getTotalMoney()
        catalogGui = loader.loadModel('phase_5.5/models/gui/catalog_gui')
        self.beanBank = DirectLabel(parent = page, relief = None, text = str(self.totalMoney), text_align = TextNode.ARight, text_scale = 0.074999999999999997, text_fg = (0.94999999999999996, 0.94999999999999996, 0, 1), text_shadow = (0, 0, 0, 1), text_pos = (0.495, -0.53000000000000003), text_font = ToontownGlobals.getSignFont(), textMayChange = True, image = catalogGui.find('**/bean_bank'), image_scale = (0.65000000000000002, 0.65000000000000002, 0.65000000000000002), scale = 0.90000000000000002, pos = (-0.75, 0.0, 0.59999999999999998))
        catalogGui.removeNode()
        del catalogGui
        self.accept(localAvatar.uniqueName('moneyChange'), self._PartyPlanner__moneyChange)
        self.accept(localAvatar.uniqueName('bankMoneyChange'), self._PartyPlanner__moneyChange)
        self.partyEditor = PartyEditor(self, page)
        self.partyEditor.request('Hidden')
        pos = self.gui.find('**/step_05_add_text_locator').getPos()
        self.elementBuyButton = DirectButton(parent = page, relief = None, text = TTLocalizer.PartyPlannerBuy, text_pos = (pos[0], pos[2]), text_scale = TTLocalizer.PPelementBuyButton, geom = (self.gui.find('**/add_up'), self.gui.find('**/add_down'), self.gui.find('**/add_rollover')), geom3_color = VBase4(0.5, 0.5, 0.5, 1.0), textMayChange = True, pos = (0.0, 0.0, 0.040000000000000001), command = self.partyEditor.buyCurrentElement)
    ...
    def _createInvitationPage(self):
        ...
        self.selectedInviteThemeLabel = DirectLabel(parent = page, relief = None, pos = self.gui.find('**/step_06_theme_locator').getPos(), text = '', text_scale = 0.059999999999999998, textMayChange = True)
        self.nextThemeButton = DirectButton(parent = page, relief = None, geom = (self.gui.find('**/topNext_button/nextButton_up'), self.gui.find('**/topNext_button/nextButton_down'), self.gui.find('**/topNext_button/nextButton_rollover')), command = self._PartyPlanner__nextTheme)
        self.prevThemeButton = DirectButton(parent = page, relief = None, geom = (self.gui.find('**/topPrevious_button/previousButton_up'), self.gui.find('**/topPrevious_button/previousButton_down'), self.gui.find('**/topPrevious_button/previousButton_rollover')), command = self._PartyPlanner__prevTheme)
        pos = self.gui.find('**/step_06_sendInvitation_locator').getPos()
        self.inviteButton = DirectButton(parent = page, relief = None, geom = (self.gui.find('**/send_up'), self.gui.find('**/send_down'), self.gui.find('**/send_rollover')), text = TTLocalizer.PartyPlannerInviteButton, textMayChange = True, text_scale = 0.050000000000000003, text_pos = (pos[0], pos[2]), command = self._PartyPlanner__handleComplete)
        return page
    ...
    def _createFarewellPage(self):
        ...
        self.closePlannerButton = DirectButton(parent = page, relief = None, geom = (self.gui.find('**/close_up'), self.gui.find('**/close_down'), self.gui.find('**/close_rollover')), text = TTLocalizer.PartyPlannerClosePlanner, text_scale = 0.055, text_pos = (pos[0], pos[2]), command = self._PartyPlanner__acceptExit)
        return page
