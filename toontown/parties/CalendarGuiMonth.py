...
class CalendarGuiMonth(DirectFrame):
    ...
    def __init__(self, parent, startingDateTime, scale = 1.0, pos = (0, 0, -0.10000000000000001), dayClickCallback = None, onlyFutureDaysClickable = False, onlyFutureMonthsClickable = False):
    ...
    def createDummyLocators(self):
        ...
        self.monthLeftLocator.setPos(-0.29999999999999999, 0, 0.65000000000000002)
        self.monthRightLocator = self.attachNewNode('monthRight')
        self.monthRightLocator.setPos(0.29999999999999999, 0, 0.65000000000000002)
    ...
    def createGuiObjects(self):
        self.monthLabel = DirectLabel(parent = self.monthLocator, relief = None, text = TTLocalizer.Months[self.startDate.month], text_scale = 0.074999999999999997, text_font = ToontownGlobals.getMinnieFont(), text_fg = (40 / 255.0, 140 / 255.0, 246 / 255.0, 1.0))
        self.yearLabel = DirectLabel(parent = self.yearLocator, relief = None, text = str(self.startDate.year), text_scale = 0.029999999999999999, text_font = ToontownGlobals.getMinnieFont(), text_fg = (140 / 255.0, 140 / 255.0, 246 / 255.0, 1.0))
        self.weekdayLabels = []
        for posIndex in xrange(7):
            adjustedNameIndex = (posIndex - 1) % 7
            self.weekdayLabels.append(DirectLabel(parent = self.weekDayLocators[posIndex], relief = None, text = TTLocalizer.DayNamesAbbrev[adjustedNameIndex], text_font = ToontownGlobals.getInterfaceFont(), text_fg = (255 / 255.0, 146 / 255.0, 113 / 255.0, 1.0), text_scale = 0.050000000000000003))
        
        self.createGuiDays()
        arrowUp = self.find('**/month_arrowR_up')
        arrowDown = self.find('**/month_arrowR_down')
        arrowHover = self.find('**/month_arrowR_hover')
        self.monthLeftArrow = DirectButton(parent = self.monthLeftLocator, relief = None, image = (arrowUp, arrowDown, arrowHover, arrowUp), image3_color = Vec4(1, 1, 1, 0.5), scale = (-1.0, 1.0, 1.0), command = self._CalendarGuiMonth__doMonthLeft)
        if self.onlyFutureMonthsClickable:
            self.monthLeftArrow.hide()
        
        self.monthRightArrow = DirectButton(parent = self.monthRightLocator, relief = None, image = (arrowUp, arrowDown, arrowHover, arrowUp), image3_color = Vec4(1, 1, 1, 0.5), command = self._CalendarGuiMonth__doMonthRight)
        
        def makeLabel(itemName, itemNum, *extraArgs):
            return DirectLabel(text = itemName, frameColor = (0, 0, 0, 0), text_scale = 0.040000000000000001)
        ...
        self.filterList = DirectScrolledList(parent = self.filterLocator, relief = None, pos = (0, 0, 0), image = None, text_scale = 0.025000000000000001, incButton_image = (arrowUp, arrowDown, arrowHover, arrowUp), incButton_relief = None, incButton_pos = filterLocatorDownPos, incButton_image3_color = Vec4(1, 1, 1, 0.20000000000000001), incButtonCallback = self.filterChanged, decButton_image = (arrowUp, arrowDown, arrowHover, arrowUp), decButton_relief = None, decButton_pos = filterLocatorUpPos, decButton_scale = (1, 1, -1), decButton_image3_color = Vec4(1, 1, 1, 0.20000000000000001), decButtonCallback = self.filterChanged, numItemsVisible = 1, itemMakeFunction = makeLabel, items = [
            TTLocalizer.CalendarShowAll,
            TTLocalizer.CalendarShowOnlyHolidays,
            TTLocalizer.CalendarShowOnlyParties], itemFrame_frameSize = (-0.20000000000000001, 0.20000000000000001, -0.02, 0.050000000000000003), itemFrame_frameColor = (0, 0, 0, 0))
        gui.removeNode()
