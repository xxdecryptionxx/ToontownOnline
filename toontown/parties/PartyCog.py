...
class PartyCog(FSM):
    ...
    def load(self):
        ...
        self.arm1Coll = CollisionSphere(1.6499999999999999, 0, 3.9500000000000002, 1.0)
        ...
        self.arm2Coll = CollisionSphere(-1.6499999999999999, 0, 3.4500000000000002, 1.0)
    ...
    def enterDown(self):
        ...
        self.hitInterval = Sequence(LerpFunc(self.setAlongSpline, duration = 1.0, fromData = self.currentT, toData = 0.0), LerpScaleInterval(self.hole, duration = 0.17499999999999999, scale = endScale, startScale = startScale, blendType = 'easeIn'), Parallel(SoundInterval(self.upSound, volume = 0.59999999999999998, node = self.actor, cutOff = PartyGlobals.PARTY_COG_CUTOFF), ActorInterval(self.actor, 'down', loop = 0)), LerpScaleInterval(self.hole, duration = 0.17499999999999999, scale = Point3(3, 3, 3), startScale = endScale, blendType = 'easeOut'))
    ...
    def exitDown(self):
        ...
        startScale = self.hole.getScale()
        endScale = Point3(5, 5, 5)
        self.hitInterval = Sequence(LerpScaleInterval(self.hole, duration = 0.17499999999999999, scale = endScale, startScale = startScale, blendType = 'easeIn'), Parallel(SoundInterval(self.upSound, volume = 0.59999999999999998, node = self.actor, cutOff = PartyGlobals.PARTY_COG_CUTOFF), ActorInterval(self.actor, 'up', loop = 0)), Func(self.actor.loop, 'idle'), LerpScaleInterval(self.hole, duration = 0.17499999999999999, scale = Point3(3, 3, 3), startScale = endScale, blendType = 'easeOut'))
        self.hitInterval.start()
    ...
    def showHitScore(self, number, scale = 1):
        ...
        seq = Task.sequence(self.hpText.lerpPos(Point3(self.root.getX(render), self.root.getY(render), self.root.getZ(render) + self.height + 1.0), 0.25, blendType = 'easeOut'), Task.pause(0.25), self.hpText.lerpColor(Vec4(r, g, b, a), Vec4(r, g, b, 0), 0.10000000000000001), Task.Task(self._PartyCog__hideHitScoreTask))
