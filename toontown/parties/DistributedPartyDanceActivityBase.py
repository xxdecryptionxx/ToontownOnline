...
class DistributedPartyDanceActivityBase(DistributedPartyActivity):
    ...
    def startActive(self):
        self.accept('enter' + DANCE_FLOOR_COLLISION, self._DistributedPartyDanceActivityBase__handleEnterDanceFloor)
        self.accept('exit' + DANCE_FLOOR_COLLISION, self._DistributedPartyDanceActivityBase__handleExitDanceFloor)
    ...
    def _DistributedPartyDanceActivityBase__handleEnterDanceFloor(self, collEntry):
    ...
    def _DistributedPartyDanceActivityBase__localStartDancing(self, h):
        if not self.localToonDancing:
            ...
            if place and hasattr(place, 'fsm'):
                ...
                place.fsm.request('activity')
                self._DistributedPartyDanceActivityBase__updateLocalToonState(ToonDancingStates.Run)
                self._DistributedPartyDanceActivityBase__setViewMode(DanceViews.Dancing)
                self.gui.load()
                self.startRules()
    ...
    def _DistributedPartyDanceActivityBase__localEnableControls(self):
        ...
        self.accept(KeyCodes.PATTERN_MATCH_EVENT, self._DistributedPartyDanceActivityBase__doDanceMove)
        self.accept(KeyCodes.PATTERN_NO_MATCH_EVENT, self._DistributedPartyDanceActivityBase__noDanceMoveMatch)
    ...
    def _DistributedPartyDanceActivityBase__handleExitDanceFloor(self, collEntry):
        if self.localToonDanceSequence is not None:
    ...
    def _DistributedPartyDanceActivityBase__localStopDancing(self):
        if self.localToonDancing:
            ...
            self.gui.unload()
            self._DistributedPartyDanceActivityBase__setViewMode(DanceViews.Normal)
            self._DistributedPartyDanceActivityBase__updateLocalToonState(ToonDancingStates.Cleanup)
    ...
    def _DistributedPartyDanceActivityBase__doDanceMove(self, pattern):
        ...
        if anim:
            self._DistributedPartyDanceActivityBase__updateLocalToonState(ToonDancingStates.DanceMove, anim)
            ...
            if pattern not in self.localPatternsMatched:
                ...
                self.localToonDanceSequence = Sequence(Func(self._DistributedPartyDanceActivityBase__localDisableControls), Parallel(camera.posInterval(0.5, Point3(0, 15, 10), blendType = 'easeIn'), camera.hprInterval(0.5, Point3(h, -20, 0), blendType = 'easeIn')), camNode.hprInterval(4.0, Point3(camNode.getH() - 360, 0, 0)), Func(camera.wrtReparentTo, camParent), Func(camNode.removeNode), Parallel(camera.posInterval(0.5, pos, blendType = 'easeOut'), camera.hprInterval(0.5, hpr, blendType = 'easeOut')), Func(self._DistributedPartyDanceActivityBase__localEnableControls))
            else:
                self.localToonDanceSequence = Sequence(Func(self._DistributedPartyDanceActivityBase__localDisableControls), Wait(2.0), Func(self._DistributedPartyDanceActivityBase__localEnableControls))
    ...
    def _DistributedPartyDanceActivityBase__noDanceMoveMatch(self):
        ...
        self._DistributedPartyDanceActivityBase__updateLocalToonState(ToonDancingStates.DanceMove)
        self.localToonDanceSequence = Sequence(Func(self._DistributedPartyDanceActivityBase__localDisableControls), Wait(1.0), Func(self._DistributedPartyDanceActivityBase__localEnableControls))
        self.localToonDanceSequence.start()
    ...
    def _DistributedPartyDanceActivityBase__updateLocalToonState(self, state, anim = ''):
