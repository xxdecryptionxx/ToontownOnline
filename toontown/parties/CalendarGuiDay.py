...
class CalendarGuiDay(DirectFrame):
    ...
    def attachMarker(self, parent, scale = 0.0050000000000000001, color = (1, 0, 0)):
        if self.showMarkers:
    ...
    def createGuiObjects(self):
        self.dayButton = DirectButton(parent = self.dayButtonLocator, image = self.selectedFrame, relief = None, command = self._CalendarGuiDay__clickedOnDay, pressEffect = 1, rolloverSound = None, clickSound = None)
        self.numberWidget = DirectLabel(parent = self.numberLocator, relief = None, text = str(self.myDate.day), text_scale = 0.040000000000000001, text_align = TextNode.ACenter, text_font = ToontownGlobals.getInterfaceFont(), text_fg = Vec4(110 / 255.0, 126 / 255.0, 255 / 255.0, 1))
        ...
        self.scrollList = DirectScrolledList(parent = self.scrollLocator, relief = None, pos = (0, 0, 0), incButton_image = (arrowUp, arrowDown, arrowHover, arrowUp), incButton_relief = None, incButton_scale = (self.arrowButtonXScale, 1, self.arrowButtonZScale), incButton_pos = incButtonPos, incButton_image3_color = Vec4(1, 1, 1, 0.20000000000000001), decButton_image = (arrowUp, arrowDown, arrowHover, arrowUp), decButton_relief = None, decButton_scale = (self.arrowButtonXScale, 1, -(self.arrowButtonZScale)), decButton_pos = decButtonPos, decButton_image3_color = Vec4(1, 1, 1, 0.20000000000000001), itemFrame_pos = (self.itemFrameXorigin, 0, -0.029999999999999999), numItemsVisible = 4, incButtonCallback = self.scrollButtonPressed, decButtonCallback = self.scrollButtonPressed)
        ...
        clipper.setPlane(Plane(Vec3(-1, 0, 0), Point3(0.23000000000000001, 0, 0)))
    ...
    def adjustForMonth(self):
        ...
        if self.onlyFutureDaysClickable:
            if not self.myDate.year < curServerDate.year:
                if (self.myDate.year == curServerDate.year or self.myDate.month < curServerDate.month or self.myDate.year == curServerDate.year) and self.myDate.month == curServerDate.month and self.myDate.day < curServerDate.day:
    ...
    def collectTimedEvents(self):
        ...
        for timedEvent in self.timedEvents:
            ...
            if isinstance(timedEvent[1], tuple) and timedEvent[1][0] == NewsManager.YearlyHolidayType:
                ...
                continue
            if isinstance(timedEvent[1], tuple) and timedEvent[1][0] == NewsManager.OncelyHolidayType:
                self.addOncelyHolidayToScrollList(timedEvent[1])
                continue
            if isinstance(timedEvent[1], tuple) and timedEvent[1][0] == NewsManager.OncelyMultipleStartHolidayType:
                self.addOncelyMultipleStartHolidayToScrollList(timedEvent[1])
                continue
            if isinstance(timedEvent[1], tuple) and timedEvent[1][0] == NewsManager.RelativelyHolidayType:
    ...
    def addTitleAndDescToScrollList(self, title, desc):
        ...
        newItem = DirectButton(relief = None, text = title, text_scale = textSize, text_align = TextNode.ALeft, rolloverSound = None, clickSound = None, pressEffect = 0, command = self._CalendarGuiDay__clickedOnScrollItem)
    ...
    def addPartyToScrollList(self, party):
        ...
        textSize = self.ScrollListTextSize
        descTextSize = 0.050000000000000003
        newItem = DirectButton(relief = None, text = partyTitle, text_scale = textSize, text_align = TextNode.ALeft, rolloverSound = None, clickSound = None, pressEffect = 0, command = self._CalendarGuiDay__clickedOnScrollItem)
        scrollItemHeight = newItem.getHeight()
        descUnderItemZAdjust = scrollItemHeight * descTextSize / textSize
        descUnderItemZAdjust = max(0.053400000000000003, descUnderItemZAdjust)
        descUnderItemZAdjust = -descUnderItemZAdjust
...
class MiniInviteVisual(DirectFrame):
    ...
    def __init__(self, parent, partyInfo):
        DirectFrame.__init__(self, parent, pos = (0.10000000000000001, 0, -0.017999999999999999))
        ...
        self.background = DirectFrame(parent = self, relief = None, geom = self.inviteBackgrounds.find('**/%s' % backgrounds[self.partyInfo.inviteTheme]), scale = (0.69999999999999996, 1.0, 0.23000000000000001), pos = (0.0, 0.0, -0.10000000000000001))
        self.whosePartyLabel = DirectLabel(parent = self, relief = None, pos = (0.070000000000000007, 0.0, -0.040000000000000001), text = ' ', text_scale = 0.040000000000000001, text_wordwrap = 8, textMayChange = True)
        self.whenTextLabel = DirectLabel(parent = self, relief = None, text = ' ', pos = (0.070000000000000007, 0.0, -0.13), text_scale = 0.040000000000000001, textMayChange = True)
        self.partyStatusLabel = DirectLabel(parent = self, relief = None, text = ' ', pos = (0.070000000000000007, 0.0, -0.17499999999999999), text_scale = 0.040000000000000001, textMayChange = True)
