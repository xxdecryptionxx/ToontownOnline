...
class CatalogGenerator:
    ...
    def generateMonthlyCatalog(self, avatar, weekStart):
        ...
        for list in itemLists:
            ...
            for item in list:
                monthlyCatalog += self._CatalogGenerator__selectItem(avatar, item, [], saleItem = saleItem)
    ...
    def generateWeeklyCatalog(self, avatar, week, monthlyCatalog):
        ...
        if week >= 1 and week <= len(WeeklySchedule):
            ...
            if nextAvailableCloset not in schedule:
                weeklyCatalog += self._CatalogGenerator__selectItem(avatar, nextAvailableCloset, monthlyCatalog, saleItem = 0)
            
            weeklyCatalog += self._CatalogGenerator__selectItem(avatar, get50ItemTrunk, monthlyCatalog, saleItem = 0)
