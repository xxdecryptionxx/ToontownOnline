...
class MailboxScreen(DirectObject.DirectObject):
    ...
    def _MailboxScreen__handleExit(self):
        if self.numAtticAccepted == 0:
            ...
            self.dialogBox = TTDialog.TTDialog(style = TTDialog.Acknowledge, text = TTLocalizer.CatalogAcceptInAttic, text_wordwrap = 15, command = self._MailboxScreen__acceptExit)
            self.dialogBox.show()
        else:
            self.dialogBox = TTDialog.TTDialog(style = TTDialog.Acknowledge, text = TTLocalizer.CatalogAcceptInAtticP, text_wordwrap = 15, command = self._MailboxScreen__acceptExit)
            self.dialogBox.show()
    ...
    def _MailboxScreen__handleDiscard(self, buttonValue = None):
        if self.acceptingIndex != None:
            ...
            self._MailboxScreen__showCurrentItem()
            item = self.items[self.itemIndex]
            item.discardItem(self.mailbox, self.acceptingIndex, self._MailboxScreen__discardItemCallback)
    ...
    def _MailboxScreen__showCurrentItem(self):
        ...
        if isinstance(item, CatalogItem.CatalogItem):
            self.acceptButton['text'] = ('', TTLocalizer.MailboxAcceptButton, TTLocalizer.MailboxAcceptButton, '')
            self.DiscardButton['text'] = ('', TTLocalizer.MailBoxDiscard, TTLocalizer.MailBoxDiscard, '')
            if item.isAward():
            ...
            self.acceptButton['text'] = ('', TTLocalizer.MailboxAcceptInvite, TTLocalizer.MailboxAcceptInvite, '')
            self.DiscardButton['text'] = ('', TTLocalizer.MailBoxRejectInvite, TTLocalizer.MailBoxRejectInvite, '')
            partyInfo = None
    ...
    def getSenderName(self, avId):
        ...
        if not sender:
            ...
            if hasattr(base.cr, 'playerFriendsManager'):
                ...
                self.accept('friendsListChanged', self._MailboxScreen__showCurrentItem)
