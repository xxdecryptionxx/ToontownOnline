...
class DistributedPetAI(DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI, PetBase.PetBase):
    ...
    def __init__(self, air, dna = None):
        ...
        self.unstickFSM = ClassicFSM.ClassicFSM('unstickFSM', [
            ...
            State.State('on', self.unstickEnterOn, self.unstickExitOn)], 'off', 'off')
    ...
    def avatarInteract(self, avId):
        ...
        self.sendUpdateToAvatarId(avId, 'avatarInteract', [
            avId])
        self.acceptOnce(self.air.getAvatarExitEvent(avId), self._DistributedPetAI__handleUnexpectedExit, extraArgs = [
            avId])
        return 1
    ...
    def _DistributedPetAI__petMovieStart(self, avId):
        ...
        taskMgr.doMethodLater(time, self._DistributedPetAI__petMovieComplete, self.uniqueName('PetMovieComplete'))
    ...
    def _DistributedPetAI__lockPetMoveTask(self, avId):
        ...
        if dist - movieDist > 0.25 and abs(avgDist - dist) > 0.10000000000000001:
            self.lockMover.move()
            taskMgr.doMethodLater(simbase.petMovePeriod, self._DistributedPetAI__lockPetMoveTask, self.getLockMoveTaskName(), [
                avId])
        else:
