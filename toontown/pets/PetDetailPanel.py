        ...
        for trickId in PetTricks.TrickId2scIds.keys():
            ...
            if trickId < len(pet.trickAptitudes):
                ...
                if aptitude != 0:
                    ...
                    if not label:
                        self.labels[trickId] = DirectLabel(parent = self, relief = None, pos = (0, 0, 0.42999999999999999 - trickId * 0.155), scale = 0.69999999999999996, text = trickText, text_scale = TTLocalizer.PDPtrickText, text_fg = (0.050000000000000003, 0.14000000000000001, 0.40000000000000002, 1), text_align = TextNode.ALeft, text_pos = (-1.3999999999999999, -0.050000000000000003))
                    else:
                        label['text'] = trickText
                    if not bar:
                        self.bars[trickId] = DirectWaitBar(parent = self, pos = (0, 0, 0.42999999999999999 - trickId * 0.155), relief = DGG.SUNKEN, frameSize = (-0.5, 0.90000000000000002, -0.10000000000000001, 0.10000000000000001), borderWidth = (0.025000000000000001, 0.025000000000000001), scale = 0.69999999999999996, frameColor = (0.40000000000000002, 0.59999999999999998, 0.40000000000000002, 1), barColor = barColor, range = 1.0 + FUDGE_FACTOR, value = length + FUDGE_FACTOR, text = str(int(hp)) + ' ' + TTLocalizer.Laff, text_scale = TTLocalizer.PDPlaff, text_fg = (0.050000000000000003, 0.14000000000000001, 0.40000000000000002, 1), text_align = TextNode.ALeft, text_pos = TTLocalizer.PDPlaffPos)
                    else:
