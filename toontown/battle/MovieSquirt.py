...
def __getSuitTrack(suit, tContact, tDodge, hp, hpbonus, kbbonus, anim, died, leftSuits, rightSuits, battle, toon, fShowStun, beforeStun = 0.5, afterStun = 1.8, geyser = 0, uberRepeat = 0, revived = 0):
    if hp > 0:
        ...
        if kbbonus > 0 and not geyser:
            ...
            sUp = LerpPosInterval(suit, 1.1000000000000001, suitEndPos, startPos = suitStartPos, fluid = 1)
            sDown = LerpPosInterval(suit, 0.59999999999999998, suitStartPos, startPos = suitEndPos, fluid = 1)
        elif fShowStun == 1:
...
def __doStormCloud(squirt, delay, fShowStun):
    ...
    if hp > 0 or delay <= 0:
        tracks.append(__getSuitTrack(suit, tContact, tSuitDodges, hp, hpbonus, kbbonus, 'soak', died, leftSuits, rightSuits, battle, toon, fShowStun, beforeStun = 2.6000000000000001, afterStun = 2.2999999999999998, revived = revived))
...
def __doGeyser(squirt, delay, fShowStun, uberClone = 0):
    ...
    for target in squirt['target']:
        ...
        if hp > 0 or delay <= 0:
            tracks.append(Sequence(Wait(delayTime), __getSuitTrack(suit, tContact, tSuitDodges, hp, hpbonus, kbbonus, 'soak', died, leftSuits, rightSuits, battle, toon, fShowStun, beforeStun = 2.6000000000000001, afterStun = 2.2999999999999998, geyser = 1, uberRepeat = uberClone, revived = revived)))
            continue
    
    return tracks

squirtfn_array = (__doFlower, __doWaterGlass, __doWaterGun, __doSeltzerBottle, __doFireHose, __doStormCloud, __doGeyser)
