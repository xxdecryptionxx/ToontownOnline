...
def __createWeddingCakeFlight(throw, groupHitDict, pie, pies):
    ...
    for i in range(numTargets):
        ...
        if hitSuit:
            ...
            pieMiss = LerpFunctionInterval(__pieMissGroupLerpCallback, extraArgs = [
                missDict], duration = (tPieHitsSuit - tPieLeavesHand) * ratioMissToHit)
            pieHide = Func(MovieUtil.removeProps, pies)
            pieRemoveCakeParts = Func(MovieUtil.removeProps, cakePartDivToUse[i])
            singlePieTrack.append(piePartReparent)
    ...
    for i in range(numTargets):
        ...
        if hitSuit:
            ...
            if groupHitValues.count(0) == len(groupHitValues):
                ...
                singleSuitResponseTrack = Sequence(Wait(tPieHitsSuit - 0.10000000000000001), Func(MovieUtil.indicateMissed, suit, 1.0))
