...
class FireCogPanel(StateData.StateData):
    ...
    def load(self):
        ...
        self.textFrame = DirectFrame(parent = self.frame, relief = None, image = gui.find('**/PckMn_Select_Tab'), image_color = Vec4(1, 1, 0, 1), image_scale = (1.0, 1.0, 2.0), text = '', text_fg = Vec4(0, 0, 0, 1), text_pos = (0, 0.02, 0), text_scale = TTLocalizer.FCPtextFrame, pos = (-0.012999999999999999, 0, 0.012999999999999999))
        self.textFrame['text'] = TTLocalizer.FireCogTitle % localAvatar.getPinkSlips()
        self.avatarButtons = []
        for i in range(4):
            button = DirectButton(parent = self.frame, relief = None, text = '', text_fg = Vec4(0, 0, 0, 1), text_scale = 0.067000000000000004, text_pos = (0, -0.014999999999999999, 0), textMayChange = 1, image_scale = (1.0, 1.0, 1.0), image = (gui.find('**/PckMn_Arrow_Up'), gui.find('**/PckMn_Arrow_Dn'), gui.find('**/PckMn_Arrow_Rlvr')), command = self._FireCogPanel__handleAvatar, extraArgs = [
                i])
            button.setScale(1, 1, 1)
            button.setPos(0, 0, 0.20000000000000001)
            self.avatarButtons.append(button)
        
        self.backButton = DirectButton(parent = self.frame, relief = None, image = (gui.find('**/PckMn_BackBtn'), gui.find('**/PckMn_BackBtn_Dn'), gui.find('**/PckMn_BackBtn_Rlvr')), pos = (-0.64700000000000002, 0, 0.0060000000000000001), scale = 1.05, text = TTLocalizer.TownBattleChooseAvatarBack, text_scale = 0.050000000000000003, text_pos = (0.01, -0.012), text_fg = Vec4(0, 0, 0.80000000000000004, 1), command = self._FireCogPanel__handleBack)
        gui.removeNode()
        self.loaded = 1
    ...
    def enter(self, numAvatars, localNum = None, luredIndices = None, trappedIndices = None, track = None, fireCosts = None):
        ...
        self._FireCogPanel__placeButtons(numAvatars, invalidTargets, localNum, fireCosts)
    ...
    def _FireCogPanel__placeButtons(self, numAvatars, invalidTargets, localNum, fireCosts):
        canfire = 0
        for i in range(4):
