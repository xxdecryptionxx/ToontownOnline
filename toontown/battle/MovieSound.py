    ...
    for target in targets:
        ...
        if totalDamage > 0 and sound == lastSoundThatHit:
            ...
            if isUber:
                ...
                suitTrack.append(Parallel(showDamage, updateHealthBar, SoundInterval(soundEffect, node = suit), __getPartTrack(breakEffect, 0.0, 1.0, [
                    breakEffect,
                    suit,
                    0], softStop = -0.5)))
            else:
...
def __createToonInterval(sound, delay, toon, operaInstrument = None):
    ...
    if operaInstrument:
        ...
        retval.append(Func(setPosFromOther, sprayEffect, operaInstrument, Point3(0, 1.6000000000000001, -0.17999999999999999)))
...
def __doBikehorn(sound, delay, toon, targets, level):
    ...
    instrMax = Vec3(0.65000000000000002, 0.65000000000000002, 0.65000000000000002)
    instrMax *= INSTRUMENT_SCALE_MODIFIER
    instrStretch = Vec3(0.59999999999999998, 1.1000000000000001, 0.59999999999999998)
    ...
    def setInstrumentStats(instrument = instrument, instrument2 = instrument2):
        instrument.setPos(-1.1000000000000001, -1.3999999999999999, 0.10000000000000001)
        instrument.setHpr(145, 0, 0)
        instrument.setScale(instrMin)
        instrument2.setPos(-1.1000000000000001, -1.3999999999999999, 0.10000000000000001)
    ...
    grow = getScaleIntervals(instruments, duration = 0.20000000000000001, startScale = instrMin, endScale = instrMax)
    instrumentAppear = Parallel(grow, Sequence(Wait(0.14999999999999999), SoundInterval(instrumentAppearSfx, node = toon)))
    stretchInstr = getScaleBlendIntervals(instruments, duration = 0.20000000000000001, startScale = instrMax, endScale = instrStretch, blendType = 'easeOut')
    backInstr = getScaleBlendIntervals(instruments, duration = 0.20000000000000001, startScale = instrStretch, endScale = instrMax, blendType = 'easeIn')
    stretchMega = getScaleBlendIntervals(megaphones, duration = 0.20000000000000001, startScale = megaphone.getScale(), endScale = 0.90000000000000002, blendType = 'easeOut')
    backMega = getScaleBlendIntervals(megaphones, duration = 0.20000000000000001, startScale = 0.90000000000000002, endScale = megaphone.getScale(), blendType = 'easeIn')
    ...
    instrumentshrink = getScaleIntervals(instruments, duration = 0.10000000000000001, startScale = instrMax, endScale = instrMin)
    if soundEffect:
        delayTime = delay + tSound
        if hasLuredSuits:
            delayTime += TIME_TO_WALK_BACK
        
        soundTrack = Sequence(Wait(delayTime), Parallel(attackTrack, SoundInterval(soundEffect, node = toon)), Wait(0.20000000000000001), instrumentshrink)
...
def __doWhistle(sound, delay, toon, targets, level):
    tracks = Parallel()
    instrMin = Vec3(0.001, 0.001, 0.001)
    instrMax = Vec3(0.20000000000000001, 0.20000000000000001, 0.20000000000000001)
    instrMax *= INSTRUMENT_SCALE_MODIFIER
    ...
    instrumentAppearSfx = globalBattleSoundCache.getSound(appearSoundFiles[level])
    grow = getScaleIntervals(instruments, duration = 0.20000000000000001, startScale = instrMin, endScale = instrMax)
    instrumentAppear = Parallel(grow, Sequence(Wait(0.050000000000000003), SoundInterval(instrumentAppearSfx, node = toon)))
    stretchInstr = getScaleBlendIntervals(instruments, duration = 0.20000000000000001, startScale = instrMax, endScale = instrStretch, blendType = 'easeOut')
    backInstr = getScaleBlendIntervals(instruments, duration = 0.20000000000000001, startScale = instrStretch, endScale = instrMax, blendType = 'easeIn')
...
def __doBugle(sound, delay, toon, targets, level):
    tracks = Parallel()
    instrMin = Vec3(0.001, 0.001, 0.001)
    instrMax = Vec3(0.40000000000000002, 0.40000000000000002, 0.40000000000000002)
    instrMax *= INSTRUMENT_SCALE_MODIFIER
    ...
    def longshake(models, num):
        inShake = getScaleBlendIntervals(models, duration = 0.20000000000000001, startScale = instrMax, endScale = instrStretch, blendType = 'easeInOut')
        outShake = getScaleBlendIntervals(models, duration = 0.20000000000000001, startScale = instrStretch, endScale = instrMax, blendType = 'easeInOut')
        i = 1
        seq = Sequence()
        while i < num:
            if i % 2 == 0:
...
def __doAoogah(sound, delay, toon, targets, level):
    ...
    instrMax *= INSTRUMENT_SCALE_MODIFIER
    instrStretch = Vec3(1.1000000000000001, 0.90000000000000002, 0.40000000000000002)
    instrStretch *= INSTRUMENT_SCALE_MODIFIER
    megaphone = globalPropPool.getProp('megaphone')
    megaphone2 = MovieUtil.copyProp(megaphone)
    megaphones = [
    ...
    instrumentAppear = Parallel(grow, Sequence(Wait(0.050000000000000003), SoundInterval(instrumentAppearSfx, node = toon)))
    stretchInstr = getScaleBlendIntervals(instruments, duration = 0.20000000000000001, startScale = instrMax, endScale = instrStretch, blendType = 'easeOut')
    backInstr = getScaleBlendIntervals(instruments, duration = 0.20000000000000001, startScale = instrStretch, endScale = instrMax, blendType = 'easeInOut')
...
def __doElephant(sound, delay, toon, targets, level):
    tracks = Parallel()
    instrMin = Vec3(0.001, 0.001, 0.001)
    instrMax1 = Vec3(0.29999999999999999, 0.40000000000000002, 0.20000000000000001)
    ...
    instrStretch2 = Vec3(0.29999999999999999, 0.69999999999999996, 0.29999999999999999)
    ...
    def setInstrumentStats(instrument = instrument, instrument2 = instrument2):
        instrument.setPos(-0.59999999999999998, -0.90000000000000002, 0.14999999999999999)
        instrument.setHpr(145, 0, 85)
        instrument.setScale(instrMin)
        instrument2.setPos(-0.59999999999999998, -0.90000000000000002, 0.14999999999999999)
        instrument2.setHpr(145, 0, 85)
        instrument2.setScale(instrMin)
    ...
    megaphoneHide = Sequence(Func(MovieUtil.removeProps, megaphones), Func(MovieUtil.removeProps, instruments))
    instrumentAppearSfx = globalBattleSoundCache.getSound(appearSoundFiles[level])
    grow1 = getScaleIntervals(instruments, duration = 0.29999999999999999, startScale = instrMin, endScale = instrMax1)
    grow2 = getScaleIntervals(instruments, duration = 0.29999999999999999, startScale = instrMax1, endScale = instrMax2)
    ...
    stretchInstr1 = getScaleBlendIntervals(instruments, duration = 0.10000000000000001, startScale = instrMax2, endScale = instrStretch1, blendType = 'easeOut')
    stretchInstr2 = getScaleBlendIntervals(instruments, duration = 0.10000000000000001, startScale = instrStretch1, endScale = instrStretch2, blendType = 'easeOut')
    stretchInstr = Sequence(stretchInstr1, stretchInstr2)
    backInstr = getScaleBlendIntervals(instruments, duration = 0.10000000000000001, startScale = instrStretch2, endScale = instrMax2, blendType = 'easeOut')
    attackTrack = Sequence(stretchInstr, Wait(1), backInstr)
    hasLuredSuits = __hasLuredSuits(sound)
    delayTime = delay
    if hasLuredSuits:
    ...
    tracks.append(toonTrack)
    soundEffect = globalBattleSoundCache.getSound(soundFiles[level])
    instrumentshrink = getScaleIntervals(instruments, duration = 0.10000000000000001, startScale = instrMax2, endScale = instrMin)
    if soundEffect:
        delayTime = delay + tSound
        if hasLuredSuits:
            delayTime += TIME_TO_WALK_BACK
...
def __doOpera(sound, delay, toon, targets, level):
    ...
    instrMax1 *= INSTRUMENT_SCALE_MODIFIER
    instrMax2 = Vec3(2.2000000000000002, 2.2000000000000002, 2.2000000000000002)
    instrMax2 *= INSTRUMENT_SCALE_MODIFIER
    instrStretch = Vec3(0.40000000000000002, 0.40000000000000002, 0.40000000000000002)
    instrStretch *= INSTRUMENT_SCALE_MODIFIER
    megaphone = globalPropPool.getProp('megaphone')
    ...
    def setInstrumentStats(instrument = instrument, instrument2 = instrument2):
        ...
        newPos = Vec3(-0.80000000000000004, -0.90000000000000002, 0.20000000000000001)
        newPos *= 1.3
    ...
    grow2 = getScaleBlendIntervals(instruments, duration = 1.1000000000000001, startScale = instrMax1, endScale = instrMax2, blendType = 'easeIn')
    shrink2 = getScaleIntervals(instruments, duration = 0.10000000000000001, startScale = instrMax2, endScale = instrMin)
...
soundfn_array = (__doBikehorn, __doWhistle, __doBugle, __doAoogah, __doElephant, __doFoghorn, __doOpera)
