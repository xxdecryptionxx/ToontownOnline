...
def __getSoundTrack(level, hitSuit, node = None):
    ...
    throwTrack = Sequence(Wait(2.1499999999999999), SoundInterval(throwSound, node = node))
    return throwTrack
...
def __throwPie(throw, delay, hitCount, showCannon = 1):
    ...
    if showCannon:
        ...
        import math as math
        suitScale = 0.90000000000000002 - math.sqrt(suitLevel) * 0.10000000000000001
        sival = []
        ...
        playSoundCannonAdjust = SoundInterval(soundCannonAdjust, duration = 0.59999999999999998, node = cannonHolder)
        ...
        reactIval = Parallel(ActorInterval(suit, 'pie-small-react'), Sequence(Wait(0.0), LerpPosInterval(cannonHolder, 2.0, posFinal, startPos = posInit, blendType = 'easeInOut'), Parallel(LerpHprInterval(barrel, 0.59999999999999998, Point3(0, 45, 0), startHpr = Point3(0, 90, 0), blendType = 'easeIn'), playSoundCannonAdjust), Wait(2.0), Parallel(LerpHprInterval(barrel, 0.59999999999999998, Point3(0, 90, 0), startHpr = Point3(0, 45, 0), blendType = 'easeIn'), playSoundCannonAdjust), LerpPosInterval(cannonHolder, 1.0, posInit, startPos = posFinal, blendType = 'easeInOut')), Sequence(Wait(0.0), Parallel(ActorInterval(suit, 'flail'), suit.scaleInterval(1.0, suitScale), LerpPosInterval(suit, 0.25, Point3(0, -1.0, 0.0)), Sequence(Wait(0.25), Parallel(playSoundCogPanic, LerpPosInterval(suit, 1.5, Point3(0, -deep, 0.0), blendType = 'easeIn')))), Wait(2.5), Parallel(playSoundBomb, playSoundFly, Sequence(Func(smoke.show), Parallel(LerpScaleInterval(smoke, 0.5, 3), LerpColorScaleInterval(smoke, 0.5, Vec4(2, 2, 2, 0))), Func(smoke.hide)), Sequence(Func(kapow.show), ActorInterval(kapow, 'kapow'), Func(kapow.hide)), LerpPosInterval(suit, 3.0, Point3(0, 150.0, 0.0)), suit.scaleInterval(3.0, 0.01)), Func(suit.hide)))
        if hitCount == 1:
            sival = Sequence(Parallel(reactIval, MovieUtil.createSuitStunInterval(suit, 0.29999999999999999, 1.3)), Wait(0.0), Func(cannonHolder.remove))
        else:
            sival = reactIval
