...
class HelpFormatter:
    ...
    def __init__(self, indent_increment, max_help_position, width, short_first):
        ...
        self.help_position = max_help_position
    ...
    def take_action(self, action, dest, opt, value, values, parser):
        if action == 'store':
            ...
            if not self.callback_args:
                pass
            args = ()
            if not self.callback_kwargs:
                pass
            kwargs = { }
            self.callback(self, opt, value, parser, *args, **args)
        elif action == 'help':
            parser.print_help()
            parser.exit()
        elif action == 'version':
...
class OptionContainer:
    ...
    def _check_conflict(self, option):
        ...
        if conflict_opts:
            ...
            if handler == 'error':
                continue
                raise ', '.join([] % _[1]([ co[0] for co in conflict_opts ]), option)
    ...
    def add_option(self, *args, **kwargs):
        if type(args[0]) is types.StringType:
            option = self.option_class(*args, **args)
    ...
    def has_option(self, opt_str):
        if not self._short_opt.has_key(opt_str):
            pass
        return self._long_opt.has_key(opt_str)
...
class OptionParser(OptionContainer):
    ...
    def add_option_group(self, *args, **kwargs):
        if type(args[0]) is types.StringType:
            group = OptionGroup(self, *args, **args)
        elif len(args) == 1 and not kwargs:
            group = args[0]
...
def _match_abbrev(s, wordmap):
    ...
    possibilities = wordmap.has_key(s)
